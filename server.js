...
// Rent Roll Chef session store: sessionId -> { id, files: [...] }
...
// ═══════════════════════════════════════════════════════════
// RENT ROLL CHEF — /api/rr/*
// ═══════════════════════════════════════════════════════════

const RR_UPLOADS_DIR = path.join(UPLOADS_DIR, 'rr')
fs.mkdirSync(RR_UPLOADS_DIR, { recursive: true })

const rrStorage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, RR_UPLOADS_DIR),
  filename: (_req, file, cb) => {
    const ext  = path.extname(file.originalname)
    const base = path.basename(file.originalname, ext)
      .replace(/[^a-z0-9_\-]/gi, '_').slice(0, 48)
...
// POST /api/rr/upload
app.post('/api/rr/upload', rrUpload.array('files', 2), async (req, res) => {
  try {
    if (!req.files || req.files.length < 2) {
      return res.status(400).json({ error: 'Please upload exactly 2 rent roll files.' })
    }

    const { parseRRFile }    = await import('./lib/rr-parser.js')
    const { detectFileRoles } = await import('./lib/rr-claude.js')

    const cheapMode = isCheapMode(req)
    const sessionId = randomUUID()

    // Parse both files
    const parsed = await Promise.all(
      req.files.map(f => parseRRFile(f.path).catch(err => ({ error: err.message, isImage: false, text: '' })))
    )

    const [file1, file2]   = req.files
    const [data1, data2]   = parsed

    // Auto-detect roles
    let roles
    try {
      roles = await detectFileRoles(data1, data2, file1.originalname, file2.originalname, cheapMode)
    } catch {
      roles = { file1Role: 'CLIENT', file2Role: 'ARGUS', confidence: 'LOW', file1Reasoning: 'Auto-detection failed.', file2Reasoning: 'Defaulted.' }
    }

    const files = [
      { id: 'file1', name: file1.originalname, path: file1.path, sizeBytes: file1.size, parsedData: data1, detectedRole: roles.file1Role, reasoning: roles.file1Reasoning },
      { id: 'file2', name: file2.originalname, path: file2.path, sizeBytes: file2.size, parsedData: data2, detectedRole: roles.file2Role, reasoning: roles.file2Reasoning },
    ]

    rrSessions.set(sessionId, { id: sessionId, files, createdAt: Date.now() })
...
app.get('/api/rr/analyze', async (req, res) => {
  ...
  const clientFile = session.files.find(f => f.id === clientFileId)
  const argusFile  = session.files.find(f => f.id === argusFileId)
  if (!clientFile || !argusFile) {
    return res.status(400).json({ error: 'Invalid clientFileId or argusFileId' })
  }
  ...
  emit('rr-start', { message: 'Chef Todd has entered the kitchen.' })

  try {
    const { analyzeRentRolls } = await import('./lib/rr-claude.js')
    const { generateRRReport } = await import('./lib/rr-reporter.js')

    const cheapMode = isCheapMode(req)

    const onProgress = ({ stage, percent, message }) => {
      emit('rr-progress', { stage, percent, message })
    }

    const analysisData = await analyzeRentRolls(
      {
        clientData: clientFile.parsedData,
        argusData:  argusFile.parsedData,
    ...
    await generateRRReport(analysisData, outputPath)

    emit('rr-complete', {
      summary:      analysisData.summary,
      property:     analysisData.property,
      ...
      tenantGroups: (analysisData.tenantGroups || []).map(g => ({
        ...
        clientTenantName: g.clientRow?.tenantName || '',
        argusTenantName:  g.argusRow?.tenantName  || '',
        evidence:       g.evidence,
      })),
...
app.get('/api/rr/download/:sessionId', (req, res) => {
  ...
  const filePath = path.join(OUTPUTS_DIR, `rr-${sessionId}.xlsx`)
  if (!fs.existsSync(filePath)) {
    return res.status(404).json({ error: 'Report not found. Run analysis first.' })
  }
  res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  res.setHeader('Content-Disposition', `attachment; filename="RentRoll-Reconciliation-${sessionId.slice(0, 8)}.xlsx"`)
