...
const LAUREN_REVIEW_SEED = [
  ...
  },
  {
    ...
    checkType: 'EXECUTION', confidence: 'HIGH',
    createdAt: '2026-06-16T12:00:00.000Z',
    suggestion: "COUNTERPART SIGNATURE PAGES — SEARCH NEIGHBORING PAGES BEFORE FLAGGING: When a signature page shows ONE party signed (Landlord or Tenant) and the OTHER party's By: line is blank, this is OFTEN counterpart execution: each party signs their own page. BEFORE flagging an EXECUTION finding, search the ±3 PDF pages around the signature page for another `IN WITNESS WHEREOF` page bearing the OPPOSITE party's signature. Two complementary counterpart pages together = full execution. Sophisticated NYC commercial leases (Park Avenue Plaza, Stockbridge, Brixmor portfolios) commonly use back-to-back counterpart pages — one with Landlord signature only, one with Tenant signature only. Only flag EXECUTION when, after RENDERING and visually checking both directions, you can confirm NO counterpart signature page exists anywhere in the document. THE COUNTERPART SEARCH IS VISUAL, NEVER TEXTUAL: counterpart signature pages are routinely scanned images with a zero-length or garbled text layer, so text extraction cannot prove a counterpart absent — 'no text found' is evidence of nothing. Inspect the actual page IMAGES of the +/-3 neighboring pages before flagging; a neighboring page with little or no extractable text is the prime counterpart candidate and must be viewed first. A text-only search that 'found no counterpart' is not a completed search.",
    rationale: "Blind-loop test 2026-06-16: Evercore Fifth Supplemental FP — agent flagged Tenant block blank on p14, but p13 had the Tenant + Guarantor counterpart signature page (Elizabeth Stevenson signed for Evercore Partners + Evercore L.P.). Standard NYC counterpart execution pattern. ADDED blind loop 7 (2026-08-11, production-tier Sonnet analysts): FP on the Duff & Phelps GA Sublease — the Sublandlord's wet-ink counterpart was a scanned page with a zero-length text layer immediately preceding the Subtenant's DocuSign page; the agent grepped all 45 pages, found nothing, and flagged EXECUTION. Loop 6's D&P and General Atlantic agents had each confirmed the same counterpart by rendering. Visual-search requirement added; same family as the corpus lesson that several signatures exist only as pixels."
  },
  {
    id: 'learning-1782000000007-blind14', source: 'blind-loop-2026-06-16', active: true,
    checkType: 'MISSING_DOCUMENT', confidence: 'HIGH',
    createdAt: '2026-06-16T12:00:00.000Z',
    suggestion: "SUB-SUBLEASE ARRANGEMENTS ARE OUT OF SCOPE FOR PRIMARY LEASE REVIEW: When a tenant's amendments reference an underlying sub-sublease, sub-sublease consent, or sub-sub-related agreement involving a different sub-sublessee, these are OUT OF SCOPE for the primary lease review. Do NOT flag them as MISSING_DOCUMENT even if substantively referenced in a recital. The reviewer's scope is the primary lease + its amendments + its Guaranty — not downstream sub-sub-leasing arrangements. THE TRIGGER IS THE PARTY TEST, AND IT IS DECISIVE — READ THE RECITAL AND IDENTIFY THE PARTIES BEFORE APPLYING THIS RULE. 'Sub-sublease' is a label, not a scope determination. If the TENANT IS NOT A PARTY (a sublease between two unrelated entities, recited only as background chain-of-title for space the tenant does not occupy), drop it. But if the TENANT IS A PARTY — as sub-sublessee, sub-sublessor, sublessee or sublessor — the instrument is IN SCOPE and its absence IS a finding, because an instrument the tenant signed is not a downstream arrangement: it creates or governs the tenant's own occupancy and the landlord holds it. Being one layer down the chain changes nothing, exactly as the sublease rule holds that the direction of the sublease is irrelevant. Watch the preposition: 'between X, as sub-sublessor, and Tenant, as sub-sublessee' means the TENANT holds that space.",
    rationale: "Blind-loop test 2026-06-16: Evercore FP — agent flagged Court Square Sub-Sublease Consent and underlying Court Square Sub-Sublease. Both involve a different sub-sublessee (Court Square Capital Management) for a different floor. Reviewer doesn't track sub-sublease relationships in the primary lease folder. CORRECTION 2026-08-12, blind spot-check round 1: the factual premise of the 2026-06-16 adjudication above was WRONG, verified against the document. The Third Supplemental recital (Fisher - Park Avenue Plaza - Evercore - 12-22-2021, p.1) reads 'that certain Sub-Sublease (the Court Square Sub-Sublease) dated as of November 4, 2021, between Court Square, as sub-sublessor, and Tenant, as sub-sublessee, for the entire rentable area of the 34th Floor of the Building.' Court Square is the sub-sublessOR and EVERCORE ITSELF is the sub-sublessee; the 34th Floor is the tenant's own premises, not a different floor. The original note that both documents involve a different sub-sublessee (Court Square Capital Management) for a different floor therefore misread the parties in both respects, and the rule was suppressing an instrument the tenant had signed. Two blind runs on this folder in loop 7 disagreed with each other on these documents, which is the signature of a rule whose trigger is ambiguous. Party test added above. The two Evercore findings remain UNADJUDICATED by the reviewer (the Evercore 2 open item) — this correction makes behaviour deterministic and consistent with the document; it does not presume the reviewer's ruling."
  },
  {
  ...
  },
  {
    ...
    checkType: 'SPECIAL_AGREEMENT', confidence: 'HIGH',
    createdAt: '2026-06-16T12:00:00.000Z',
    suggestion: "STRICT NO-EXCEPTIONS FOR SNDA / NON-DISTURBANCE CLASS DOCUMENTS (extends Learning 9): SNDA, Subordination Agreement, Non-Disturbance Agreement (NDA), Landlord's Non-Disturbance Agreement, Recognition Agreement, Subordination Non-Disturbance and Attornment Agreement, Tri-Party Non-Disturbance, or anything similarly named are NEVER findings — missing, present, executed, unexecuted, all fine. This applies EVEN WHEN the agreement is explicitly referenced by name in a recital or section of another document as 'that certain [X] dated [date]'. If the document name contains 'non-disturbance' / 'subordination' / 'attornment' / 'SNDA' / 'recognition', drop the finding on sight. Learning 9 only covered pure 'SNDA' label — this strengthens it to all variant names. BUT THIS IS A TEST ON THE INSTRUMENT'S OWN TITLE, NOT ON WHAT ITS CLAUSES DO. An instrument titled Consent to Sublease, Consent to Assignment or Landlord's Consent is a CONSENT — it does not become an SNDA because one of its sections also grants the subtenant non-disturbance or recognition protection. Combined consent-and-non-disturbance instruments are ordinary and common, and reading the function rather than the title turns this rule into a blanket suppressor of exactly the consents the reviewer asks for. Ask only whether the DOCUMENT'S OWN NAME is in the SNDA family; if it is named a Consent, this rule does not reach it, whatever protections it confers.",
  ...
  },
  {
    ...
    checkType: 'MISSING_EXHIBIT', confidence: 'HIGH',
    createdAt: '2026-06-17T12:00:00.000Z',
    suggestion: "AMENDMENT SUB-EXHIBITS DESCRIBING DIAGRAMS / DEPICTIONS — DO NOT flag: When an amendment's body section references a sub-exhibit that is a diagram, depiction, or floor-plan-style visual (e.g., 'Exhibit A attached hereto depicting the additional parking spaces', 'Exhibit B showing the new floor plan'), and that diagram is not physically attached to the amendment's PDF, do NOT generate a MISSING_EXHIBIT finding. Diagrammatic / depiction sub-exhibits to amendments are routinely tracked operationally rather than as missing documents. EXCEPTIONS: (a) the sub-exhibit is a Guaranty form (still flag per F-NONSIG-EXHIBITS / Learning 15), or (b) the diagram contains the only description of premises the rent roll requires — in that case L16 fires for the underlying premises description, not for the diagram itself. GENERALISED — THE LOAD-BEARING TEST APPLIES TO ANY PLAN, DRAWING OR DEPICTION EXHIBIT, NOT ONLY AMENDMENT SUB-EXHIBITS. Before flagging ANY missing plan-type exhibit (site plan, grading or utility plan, parking layout, floor plan, sign depiction, redesign sketch), COUNT the live provisions across the whole file that depend on it and WRITE THE COUNT DOWN. A drawing named ONCE, by a single provision describing a one-time operational task, is not tracked — do not flag it. A drawing relied on by MULTIPLE live provisions (boundaries, use restrictions, protected areas, parking fields, exclusivity radii, tax parcels, pylon or signage locations) is LOAD-BEARING and its absence IS a finding; neither 'have not been approved and are not attached hereto' nor 'shall be attached upon approval' changes that, because the file's own repeated reliance on the exhibit establishes it is operative whatever a boilerplate attachment clause says. THE COUNT IS OWED WHENEVER YOU DROP AN EXHIBIT, WHATEVER RULE YOU CITE — the supersession rule and the contemplated-only rule converge on the same question, and reaching for a different rule does not excuse skipping the count; that is exactly how a load-bearing exhibit gets dropped, not by failing the test but by never being made to take it.",
  ...
  },
  {
    ...
    checkType: 'LEASE_CURRENCY', confidence: 'HIGH',
    createdAt: '2026-08-04T12:00:00.000Z',
    suggestion: "A LONG UNCOVERED GAP IN THE TERM CHAIN IS ITS OWN FINDING, EVEN ALONGSIDE MISSING-DOCUMENT FINDINGS: The anti-double-counting rule (do not add LEASE_CURRENCY when an EXECUTION defect on an unsigned amendment already IS the gap) is narrow and does NOT apply when the chain has a genuine multi-year hole covered by no document of any kind. RULE: walk the chain chronologically; if Document N's term ends on X and the next document's term begins on Y, and the period [X, Y] exceeds 6 months, emit a LEASE_CURRENCY finding worded 'Document extending Term from [X] to [Y minus 1 day]' IN ADDITION TO any MISSING_DOCUMENT findings naming the specific instruments believed to belong in that hole. The reviewer wants both: the missing-instrument findings name what can be identified, and the chain-gap finding states the period that is unaccounted for. Naming a missing First Amendment does NOT discharge the obligation to report the uncovered period. THE TEST IS SPAN, NOT MERELY NAMING: a named missing instrument discharges the gap ONLY if that instrument, on the dates the file itself recites, would actually COVER the uncovered period. Numbered placeholders ('Amendment No. 1, if any' between an Original Lease and an in-folder Amendment No. 3) DO span by construction, so no companion range finding is written there. But an instrument dated BEFORE the gap opens cannot have closed it, nor can one whose term the file never establishes, nor a collateral instrument (sublease, consent, guaranty, license) that does not extend the prime term — in those cases emit the range finding IN ADDITION to the missing-document findings. If you are genuinely unsure whether the named instrument covers the period, EMIT the range finding: a duplicate row costs one glance, a deleted correct finding is invisible.",
    rationale: "Lawyer test 2026-08-03 Amberjack: the Prime Lease runs to ~4/30/2024 and the Short Term Lease begins 6/1/2026, a 762-day hole. The reviewer's list contains 'Document extending Term from 5/1/24-5/31/26' as a separate confirmed finding sitting alongside the missing First Amendment and the missing Sublease. In blind loop 1 the agent explicitly folded the gap into those two findings and dropped it — a recall miss. REGRESSED AGAIN in blind loop 7 (2026-08-11, production-tier Sonnet analysts): the agent drafted the 5/1/24-5/31/26 range finding and then deleted it, reasoning that the missing First Amendment dated 1/28/21 was an 'already-named cause' of the gap. It is not — that amendment predates the hole by three years and cannot have closed it. Root cause was a textual contradiction between this rule and the LEASE_CURRENCY reconciliation rule, whose precedence clause suppressed the companion finding whenever any numbered amendment had been named; 'First Amendment' reads as numbered. Resolved by the span test above, which preserves the true-negative case (numbered placeholders that do span) while restoring this finding."
  },
  {
    id: 'learning-1785801600003-blind25', source: 'lawyer-test-2026-08-03', active: true,
    checkType: 'MISSING_DOCUMENT', confidence: 'HIGH',
    createdAt: '2026-08-04T12:00:00.000Z',
    suggestion: "A REFERENCED DOCUMENT THAT THE REFERENCING TEXT SAYS HAS TERMINATED OR EXPIRED IS NEVER MISSING: Before flagging any referenced instrument as a missing document, read the sentence that references it. Lease and amendment recitals routinely name prior agreements for the express purpose of RETIRING them. If the referencing text states the instrument terminates, expires, is superseded, is of no further force and effect, or has been replaced, then its absence from the folder is intentional and correct — DO NOT flag it. Trigger phrases to check for in the referencing sentence: 'terminates pursuant to its terms', 'shall be of no further force or effect', 'is hereby terminated', 'expired by its terms', 'is superseded and replaced by', 'shall automatically terminate upon'. This is the reference-side companion to the superseded-documents filter: that filter drops superseded documents after the fact, this rule stops the finding from ever being drafted. HARD LIMIT — THIS RULE DEMANDS TOTALITY, AND A PARTIAL RETIREMENT IS NOT TOTALITY. An instrument is only retired if it is retired ENTIRELY. A clause that deletes and replaces something FOR ONE PURPOSE does not retire it for the others. Before dropping any instrument or exhibit under this rule, COUNT AND READ ITS SURVIVING CROSS-REFERENCES ACROSS THE WHOLE FILE. An exhibit cited once, by the very clause that replaces it, is genuinely superseded. An exhibit cited a dozen times throughout the lease — for use restrictions, protected areas, permitted-use carve-outs, parking fields, tax-parcel boundaries, exclusivity radii — is NOT superseded merely because one later amendment swapped out the drawing. Watch specifically for replacement language that is EXPRESSLY SCOPED: 'except for the purpose of determining Rent', 'solely for purposes of the expansion option', 'for purposes of determining the Demised Premises'. PROCEDURE — the phrase 'deleted in its entirety' most often precedes a wrongly-dropped finding, so it triggers mandatory written work before you may drop anything: quote the ENTIRE replacement sentence including every trailing qualifier, because the exception clause is routinely appended AFTER the words 'in its entirety and replaced' and stopping your quotation there is how the scoping gets lost; then count the exhibit's surviving cross-references across the whole file and state that number. Drop only if you can write both a full sentence carrying NO scoping qualifier and a cross-reference count consistent with the exhibit serving a single purpose. 'Deleted in its entirety' describes how the drawing was removed, not how many of the exhibit's jobs the replacement took over. Scoped replacement is not retirement. The test is not 'is it referenced anywhere' but 'does any STILL-LIVE PROVISION DEPEND ON IT for something the replacement does not supply.' The reciprocal error also exists: do not manufacture a finding for an instrument that genuinely was retired in full. LIKEWISE FOR TERMINATION — a termination that has NOT YET TAKEN EFFECT is not a termination. Compare the surrender date to today's date. If the surrender date is in the future, even by a few weeks, the tenant is still in possession and the file still has forward obligations; every normal check runs at full strength. A notice of termination, an exercised termination option, or an agreement fixing a surrender date months out is a SCHEDULED ending, not a REACHED one. A mere right to terminate, or a notice of intent to terminate, is never a trigger. And a termination reaches only the ancestral chain — the terminated lease and its own amendments — never collateral instruments such as subleases, assignments, guaranties, licenses, or the consents that name them. PROCEDURAL REQUIREMENT: if you find yourself assembling findings and then discarding them under this rule, stop and re-check the totality and date conditions explicitly, in writing, against the actual dates. A run that ends with ZERO findings on a folder holding a documented multi-instrument chain is a claim that needs more evidence than a run that ends with findings.",
    rationale: "Blind loop 1 regression 2026-08-04, Bank of Houston: the agent flagged 'Temporary Lease Agreement dated 5/9/2018' as MISSING_DOCUMENT. Rider No. 4 Section E of the lease reads '...the term of that certain Temporary Lease Agreement dated May 9, 2018 between Landlord and Tenant ... terminates pursuant to its terms as of the commencement of Tenant's lease of the Temporary Premises.' The document is named and dated, which makes it look like a textbook referenced-document hit, but the clause exists to extinguish it. This was the only new false positive introduced in loop 1. HARD LIMIT ADDED 2026-08-05 from blind RECALL loop 6, which inverted the test and hunted false NEGATIVES. This rule, written to fix a false positive, turned out to be deleting correct findings — silently, because a suppressed finding leaves no trace in the output. Two independent failures, same shape: the rule demands totality and the agent accepted something partial. (1) A tenant folder returned ZERO findings; the agent had correctly identified all four defects and then discarded every one of them, because a termination agreement fixed a surrender date 27 days in the FUTURE and 'scheduled to end' was read as 'ended'. (2) Another agent dropped a missing site-plan exhibit — the single documented defect for that subject — because an amendment said it was 'deleted in its entirety and replaced', when the replacement was expressly scoped ('except for the purpose of determining Rent') and the exhibit still carried the tax-parcel boundary, parking-light designations, pylon locations and use carve-outs relied on by twenty surviving cross-references. Both were fixed by the totality and date limits above and re-verified: 4/4 and the exhibit correctly flagged. Regression-checked against the true-positive suppressions this rule exists for, which still fire. GENERAL LESSON: any rule phrased 'if X is entirely/wholly true, suppress' needs an explicit statement of what PARTIAL X looks like and that partial X does not fire it. CORRECTION 2026-08-12, blind spot-check round 4: the Wal-Mart instance cited above is FACTUALLY WRONG and must not be relied on as precedent. Fourth Amendment Section 3 reads verbatim: 'The original site plan attached as Exhibit C to the Original Lease is hereby deleted in its entirety and replaced with the Site Plan attached hereto as Exhibit A. Except for the purpose of determining Rent payable by Lessee pursuant to the terms of the Lease, the Demised Premises for all purposes of the Lease shall (i) consist of the Original Demised Premises and the Expansion Premises...'. The exhibit-replacement is a COMPLETE SENTENCE ending in a period; the 'except for the purpose of determining Rent' clause OPENS THE FOLLOWING SENTENCE and governs the Demised-Premises definition, NOT the exhibit replacement. So that replacement is unqualified on its face, and the loop-6 claim that it was 'expressly scoped' misattributed the carve-out to the wrong sentence. Four consecutive blind runs declined to flag Exhibit C, the most recent after counting 14 surviving cross-references, quoting the full replacement sentence and visually verifying the replacement exhibit is complete. The partial-supersession PRINCIPLE stated above remains sound and is independently supported by the other cited instance, but this instance is withdrawn as support for it, and the corresponding answer-key entry is referred to the reviewer rather than treated as a model failure. Do not tune against it in the meantime."
  },
  {
...
function seedLaurensLearnings() {
  ...
  const seedById = new Map(LAUREN_REVIEW_SEED.map(l => [l.id, l]))
  const existingIds = new Set(existing.map(l => l.id))
  const toAdd = LAUREN_REVIEW_SEED.filter(l => !existingIds.has(l.id))

  // Reconcile the TEXT of seed rules that are already in the store.
  //
  // This used to be an insert-only seed: rules were added when their id was
  // missing and otherwise left untouched forever. That silently pinned every
  // deployment to whatever wording was first written to the persistent volume,
  // so corrections to a seed rule's text never reached production even though
  // the code looked updated. A rule that is WRONG is worse than a rule that is
  // missing, because it actively suppresses correct findings, so the fix must
  // be able to travel.
  //
  // We overwrite only the authored fields. `active` is preserved from the store
  // so that a reviewer who deliberately switched a rule off does not have it
  // switched back on by a deploy.
  const AUTHORED = ['suggestion', 'rationale', 'checkType', 'confidence', 'source']
  let updated = 0
  const merged = existing.map(l => {
    const seed = seedById.get(l.id)
    if (!seed) return l // reviewer-authored learning, never touched by the seeder
    const diverged = AUTHORED.some(k => seed[k] !== undefined && l[k] !== seed[k])
    if (!diverged) return l
    updated++
    const next = { ...l }
    for (const k of AUTHORED) if (seed[k] !== undefined) next[k] = seed[k]
    next.seedSyncedAt = new Date().toISOString()
    return next
  })

  if (toAdd.length > 0 || updated > 0) {
    writeLearnings([...merged, ...toAdd])
    if (toAdd.length > 0) {
      console.log(`[learnings] Seeded ${toAdd.length} lawyer-validated rules from Lauren's 4/14/2026 TP2 review`)
    }
    if (updated > 0) {
      console.log(`[learnings] Re-synced ${updated} seed rule(s) whose text changed in code`)
    }
  }
}
