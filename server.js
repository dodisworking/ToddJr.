...
const LAUREN_REVIEW_SEED = [
  ...
  },
  {
    ...
    checkType: 'MISSING_PAGES', confidence: 'HIGH',
    createdAt: '2026-04-29T00:00:00.000Z',
    suggestion: "Use CONTENT-BASED signals to detect genuinely missing pages — never rely on page number arithmetic alone. Page numbers are structurally unreliable (dual numbering, exhibit restarts, cover pages, OCR scan errors). Instead, run these four content checks: (1) TABLE OF CONTENTS — if a TOC exists, treat it as the authoritative checklist. For every section, article, or exhibit listed, verify that substantive content actually exists for it somewhere in the document. A TOC entry with no corresponding content is a confirmed gap. (2) SENTENCE/CLAUSE CONTINUITY — at every major structural break, verify the text transitions coherently. A page that ends mid-sentence and resumes in an unrelated section on the next page means content was cut. (3) ARTICLE AND SECTION SEQUENCE — scan all article and section numbers present in the document. If the numbering skips (Article 1, 2, 4 with no Article 3; or Section 5.1, 5.2, 5.4 with no 5.3), that is a structural gap confirming real content is missing. (4) SIGNATURE PAGE — every executed lease must contain an 'IN WITNESS WHEREOF' clause followed by actual signature blocks. If the lease body ends with no signature section anywhere in the folder, pages are missing. If NONE of these four content signals fire, the document is complete — do not flag missing pages regardless of what PDF page numbers show.",
    rationale: "Synthesized from ongoing false-positive pattern. Root cause: model was checking page NUMBER sequences (unreliable) instead of content integrity (reliable). The four content signals — TOC verification, sentence continuity, section number sequence, and signature page presence — cover every real case of missing pages seen in practice. If a document passes all four checks it is complete; page number gaps at that point are formatting artifacts."
  }
]
