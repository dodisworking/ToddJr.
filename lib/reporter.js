...
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  function makeTPSheet(name, tabArgb, extraColHeader, extraColWidth) {
    const totalCols = extraColHeader ? 8 : 7
    ...
    ws.columns = [
      ...
      { key: 'severity',  width: 12 },
      { key: 'checkType', width: 20 },
      ...(extraColHeader ? [{ key: 'extra', width: extraColWidth || 45 }] : [])
    ]
    const headers = [
      'Property Name', 'Tenant Name', 'Suite Number',
      'Missing Document', 'Comment / Status', 'Severity', 'Check Type'
    ...
    headerRow.height = 24
    headerRow.eachCell((cell, colNum) => {
      const isExtra = extraColHeader && colNum === totalCols
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: isExtra ? PALETTE.inputHeaderBg : PALETTE.headerBg } }
      cell.font = { bold: true, color: { argb: PALETTE.headerFont }, size: 11, name: 'Calibri' }
      cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true }
      cell.border = { bottom: { style: 'medium', color: { argb: isExtra ? PALETTE.inputBorder : 'FF93C5FD' } } }
    })
    ws.views = [{ state: 'frozen', ySplit: 1 }]
  ...
  function addTPRow(ws, data, severity, rowIdx, extraVal) {
    ...
    if (extraVal !== undefined) vals.push(extraVal || '')
    const row = ws.addRow(vals)
    row.height = Math.max(18, Math.min(120, estimateRowHeight(missingText, commentText)))
    const colors = getSeverityColors(severity)
    const isAlt  = rowIdx % 2 === 1
    row.eachCell((cell, colNum) => {
      const isExtra = extraVal !== undefined && colNum === vals.length
      if (isExtra) {
        cell.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: PALETTE.inputCellBg } }
  ...
  for (const t of tenantResults) {
    ...
    for (const ann of annotations) {
      ...
      if (ann.cropDataUrl) embedShot(wsRej, row, ann.cropDataUrl, 8)
      rejCount++
    }
  }
