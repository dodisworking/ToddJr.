...
/**
 * Generate a 2-tab Excel covering an entire Target Practice session.
 * Tab 1: All confirmed findings across all tenants
 * Tab 2: All rejected findings across all tenants
 */
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  const wb = new ExcelJS.Workbook()
  wb.creator = 'Todd Jr. — Target Practice Session'
  wb.created = new Date()

  const HEADER_FILL  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0F172A' } }
  const HEADER_FONT  = { name: 'Calibri', bold: true, color: { argb: 'FFFFFFFF' }, size: 10 }
  const BODY_FONT    = { name: 'Calibri', size: 10 }
  const CONFIRM_FILL = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDCFCE7' } }
  const REJECT_FILL  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } }
  const ANNO_FILL    = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEF9C3' } }
  const SEV_COLORS   = { HIGH: 'FFDC2626', MEDIUM: 'FFF97316', LOW: 'FFEAB308', NONE: 'FF6B7280' }

  // Extra cols: confirmed tab has Screenshot col (8); rejected tab has Reviewer Comment (8)
  function makeSheet(name, tabArgb, extraColHeader, extraColWidth) {
    ...
    ws.columns = [
      ...
      { width: 10 }, { width: 30 }, { width: 45 }, { width: 45 },
      ...(extraColHeader ? [{ width: extraColWidth || 45 }] : [])
    ]
    const totalCols = extraColHeader ? 8 : 7
    // Title row
    ws.mergeCells(1, 1, 1, totalCols)
    const titleCell = ws.getRow(1).getCell(1)
    titleCell.value = `${name} — ${tenantResults.length} tenant${tenantResults.length !== 1 ? 's' : ''} — Reviewed by: ${reviewerName} — ${new Date().toLocaleDateString()}`
    titleCell.fill = HEADER_FILL
    titleCell.font = { ...HEADER_FONT, size: 11 }
    titleCell.alignment = { vertical: 'middle', horizontal: 'left' }
    ws.getRow(1).height = 22
  ...
  function addFindingRow(ws, property, tenantName, suite, f, fill, extraVal) {
    const vals = [property, tenantName, suite, f.severity || '', f.checkType || '', f.missingDocument || '', f.comment || '']
    ...
    sevCell.font = { ...BODY_FONT, bold: true, color: { argb: SEV_COLORS[f.severity] || 'FF6B7280' } }
    row.height = 18
    return row
  }

  function embedScreenshot(ws, row, cropDataUrl) {
  ...
  const wsConfirmed = makeSheet('✅ Confirmed Findings', '10B981', 'Flag Screenshot', 48)
  const wsRejected  = makeSheet('❌ Rejected Findings',  'EF4444', 'Reviewer Comment', 45)
  ...
  for (const t of tenantResults) {
    ...
    for (const f of confirmedFindings) {
      addFindingRow(wsConfirmed, property, tenantName, suite, f, CONFIRM_FILL, '')
      confirmedCount++
    }

    // Annotation rows — manually flagged screenshots
    for (const ann of annotations) {
      const f = {
        severity: 'NOTE', checkType: 'MANUAL FLAG',
        missingDocument: `Flag — ${ann.docName || 'Document'} · p.${ann.pageNum ?? '?'}`,
        comment: ann.comment || ''
      }
      const row = addFindingRow(wsConfirmed, property, tenantName, suite, f, ANNO_FILL, '')
      if (ann.cropDataUrl) embedScreenshot(wsConfirmed, row, ann.cropDataUrl)
      confirmedCount++
    }

    for (const f of rejectedFindings) {
      addFindingRow(wsRejected, property, tenantName, suite, f, REJECT_FILL, f.reviewerNote || '')
      rejectedCount++
    }
  }

  if (confirmedCount === 0) wsConfirmed.addRow(['No confirmed findings across this session', '', '', '', '', '', '', ''])
  if (rejectedCount  === 0) wsRejected.addRow( ['No rejected findings across this session',  '', '', '', '', '', '', ''])
