...
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  // Tab is red if anything looks off, green if everything reconciles.
  {
    ...
    const orphanTenants = []  // in output but not in input (shouldn't happen)
    for (const [key] of outputTenantCounts) {
      if (!inputTenantNames.has(key) && inputTenantNames.size > 0) {
