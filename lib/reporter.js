...
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  function makeSheet(name, tabArgb, hasReviewerCol) {
    ...
    ws.columns = [
      ...
      ...(hasReviewerCol ? [{ width: 45 }] : [])
    ]
    // Title row
    ws.mergeCells(1, 1, 1, hasReviewerCol ? 8 : 7)
    const titleCell = ws.getRow(1).getCell(1)
    ...
    // Header row
    const headers = ['Property', 'Tenant Name', 'Suite', 'Severity', 'Check Type', 'Missing Document', 'Comment / Status']
    if (hasReviewerCol) headers.push('Reviewer Comment')
    const hdrRow = ws.addRow(headers)
    hdrRow.eachCell(cell => {
  ...
  function addFindingRow(ws, property, tenantName, suite, f, fill, reviewerComment) {
    const vals = [property, tenantName, suite, f.severity || '', f.checkType || '', f.missingDocument || '', f.comment || '']
    if (reviewerComment !== undefined) vals.push(reviewerComment || '')
  ...
  const wsConfirmed = makeSheet('✅ Confirmed Findings', '10B981', false)
  const wsRejected  = makeSheet('❌ Rejected Findings',  'EF4444', true)
  ...
  for (const t of tenantResults) {
    ...
    for (const ann of annotations) {
      const f = { severity: 'NOTE', checkType: 'MANUAL FLAG', missingDocument: `${ann.docName || ''}, p.${ann.pageNum || ''}`, comment: ann.comment || '' }
      addFindingRow(wsConfirmed, property, tenantName, suite, f, ANNO_FILL)
      confirmedCount++
    }
