...
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  // ── Upload Issues sheet (only if there's something to flag) ─────────────
  // Surfaces duplicate-tenant merges, empty-tenant drops, file-count gaps,
  // and the final tenant manifest. Reviewer sees this in the Excel itself
  // — no need to dig through Railway logs to know whether the upload was
  // clean or not.
  if (uploadDiagnostics) {
    ...
    // Always include the sheet (even when clean) so the reviewer can confirm
    // the upload was processed correctly. Tab color flips red on issues.
    const tabArgb = hasIssues ? 'FFDC2626' : 'FF16A34A'  // red if issues, green if clean
    const wsDiag = wb.addWorksheet(hasIssues ? '⚠ Upload Issues' : '✓ Upload OK', {
      tabColor: { argb: tabArgb }
    })
    ...
    const titleRow = wsDiag.addRow([
      hasIssues ? '⚠ UPLOAD ISSUES — REVIEWER PLEASE READ' : '✓ UPLOAD WAS CLEAN',
      '', ''
    ])
    ...
    wsDiag.addRow(['Folders Detected', d.foldersDetected ?? '—', ''])
    wsDiag.addRow(['Final Tenants After Dedup', (d.finalTenantList?.length || 0), ''])
    wsDiag.addRow(['', '', ''])

    // Section: duplicates merged
    const dupHdr = wsDiag.addRow(['DUPLICATE TENANTS MERGED', 'Detail', 'Files Added'])
    dupHdr.eachCell(c => { c.fill = headerFill; c.font = headerFont })
    if ((d.duplicatesMerged?.length || 0) === 0) {
      const r = wsDiag.addRow(['(none)', 'No duplicate tenants detected.', ''])
      r.eachCell(c => { c.fill = okFill })
    } else {
      for (const m of d.duplicatesMerged) {
        const r = wsDiag.addRow([
          `"${m.duplicate}"`,
          `merged into "${m.mergedInto}"`,
          m.filesAdded
        ])
    ...
    // Section: empty tenants dropped
    const emptyHdr = wsDiag.addRow(['EMPTY TENANTS DROPPED', 'Folder Name', ''])
    emptyHdr.eachCell(c => { c.fill = headerFill; c.font = headerFont })
    ...
    // Section: file count delta
    const fcHdr = wsDiag.addRow(['FILE COUNT INTEGRITY', 'Status', 'Delta'])
    fcHdr.eachCell(c => { c.fill = headerFill; c.font = headerFont })
    if ((d.fileCountDelta || 0) === 0) {
    ...
    // Section: final tenant manifest
    const tHdr = wsDiag.addRow(['FINAL TENANT MANIFEST', 'Property / Suite', 'File Count'])
    tHdr.eachCell(c => { c.fill = headerFill; c.font = headerFont })
    if ((d.finalTenantList?.length || 0) === 0) {
      wsDiag.addRow(['(no tenants)', '', ''])
    } else {
