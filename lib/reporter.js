...
export async function generateReport(allFindings, outputPath) {
  ...
  const headers = [
    ...
    'Confirm or Deny Findings', 'Notes / Reason', 'Findings Comments'
  ]
...
function addDataRow(ws, data, severity, rowIndex) {
  const row = ws.addRow([
    ...
    '',   // Col J — Findings Comments (reviewer fills in)
  ])
...
// ── Session-level download: all tenants combined ──────────────────────────────
/**
 * Generate a 3-tab Excel for a Target Practice session.
 * Matches the visual style of the main Missing Documents Report.
 * Tab 1: ✅ Confirmed Findings
 * Tab 2: ❌ Rejected Findings (includes partial ◐)
 * Tab 3: Summary
 */
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  const { tenantResults = [], reviewerName = 'Unknown', juiceRules = [], uploadDiagnostics = null } = opts
  const wb = new ExcelJS.Workbook()
  wb.creator = 'Todd Jr. — Target Practice'
  wb.created = new Date()
  wb.properties.date1904 = false
  ...
  //   A–G: core AI output (unchanged)
  //   H:   user-input column (Flag Screenshot on confirmed tab, Reviewer Note on rejected tab)
  //   I:   AI Evidence — raw citation field, read-only reference for training analysis
  function makeTPSheet(name, tabArgb, extraColHeader, extraColWidth) {
    const totalCols = 9  // always 9: 7 core + 1 user-input + 1 AI Evidence
    const ws = wb.addWorksheet(name, {
      tabColor: { argb: tabArgb },
      pageSetup: { orientation: 'landscape', fitToPage: true, fitToWidth: 1, fitToHeight: 0, paperSize: 9 },
      headerFooter: {
        oddHeader: `&C&B&14Todd Jr. — ${name}`,
        oddFooter:  `&LReviewed by: ${reviewerName}  |  ${dateStr}&RPage &P of &N`
      }
    })
    ws.columns = [
      { key: 'property',  width: 16 },
      ...
      { key: 'severity',  width: 12 },
      { key: 'checkType', width: 20 },
      { key: 'extra',     width: extraColWidth || 45 },  // col H — user-input
      { key: 'evidence',  width: 55 }                    // col I — AI Evidence (read-only)
    ]
    const headers = [
      'Property Name', 'Tenant Name', 'Suite Number',
      'Missing Document', 'Comment / Status', 'Severity', 'Check Type',
    ...
    headerRow.height = 24
    headerRow.eachCell((cell, colNum) => {
      const isUserInput = colNum === 8                    // col H
      const isEvidence  = colNum === 9                    // col I
      if (isEvidence) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E3A5F' } }  // deep navy
        cell.font = { bold: true, color: { argb: PALETTE.headerFont }, size: 10, name: 'Calibri' }
      } else if (isUserInput) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: PALETTE.inputHeaderBg } }
        cell.font = { bold: true, color: { argb: PALETTE.headerFont }, size: 11, name: 'Calibri' }
      } else {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: PALETTE.headerBg } }
        cell.font = { bold: true, color: { argb: PALETTE.headerFont }, size: 11, name: 'Calibri' }
      ...
      cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true }
      cell.border = { bottom: { style: 'medium', color: { argb: isUserInput ? PALETTE.inputBorder : isEvidence ? 'FF3B82F6' : 'FF93C5FD' } } }
    })
    ws.views = [{ state: 'frozen', ySplit: 1 }]
  ...
  // ── Row adder — same style as main report ───────────────────
  function addTPRow(ws, data, severity, rowIdx, extraVal) {
    const commentText = buildCommentText(data)
    const missingText = (!data.missingDocument || data.missingDocument === 'N/A')
      ? (data.comment || '') : data.missingDocument
    const evidenceText = (data.evidence && data.evidence !== 'N/A') ? data.evidence.trim() : ''
    const vals = [
      data.property, data.tenant, String(data.suite || ''),
      missingText, commentText,
      data.severity || severity || '',
      CHECK_LABELS[data.checkType] || data.checkType || '',
      extraVal || '',    // col H — user-input (reviewer note / flag screenshot)
      evidenceText       // col I — AI Evidence (read-only)
    ]
    const row = ws.addRow(vals)
    row.height = Math.max(18, Math.min(120, estimateRowHeight(missingText, commentText)))
    const colors = getSeverityColors(severity)
    const isAlt  = rowIdx % 2 === 1
    row.eachCell((cell, colNum) => {
      const isUserInput = colNum === 8
      const isEvidence  = colNum === 9
      if (isEvidence) {
        // Read-only reference column — muted blue-gray styling
        cell.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0F4FF' } }
        cell.font      = { color: { argb: 'FF374151' }, size: 9, name: 'Calibri', italic: true }
        cell.border    = { bottom: { style: 'thin', color: { argb: 'FFD1D5DB' } }, left: { style: 'medium', color: { argb: 'FF3B82F6' } } }
        cell.alignment = { vertical: 'top', wrapText: true }
      } else if (isUserInput) {
        cell.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: PALETTE.inputCellBg } }
        cell.font      = { color: { argb: PALETTE.inputCellFont }, size: 10, name: 'Calibri' }
        cell.border    = { bottom: { style: 'thin', color: { argb: PALETTE.inputBorder } }, left: { style: 'thin', color: { argb: PALETTE.inputBorder } } }
        cell.alignment = { vertical: 'top', wrapText: true }
      } else {
        const bgColor = isAlt && severity === 'OK' ? PALETTE.altRow : colors.bg
        cell.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: bgColor } }
    ...
    row.getCell(6).font = { ...row.getCell(6).font, bold: true }
    // Suite as text
    const sc = row.getCell(3); sc.value = String(data.suite || ''); sc.numFmt = '@'
    return row
  }

  // ── Screenshot embed ─────────────────────────────────────────
  function embedShot(ws, row, cropDataUrl, colIdx) {
    if (!cropDataUrl || !String(cropDataUrl).startsWith('data:image')) return
    const m = String(cropDataUrl).match(/^data:image\/(png|jpeg|jpg|gif);base64,(.+)$/i)
    if (!m) return
    ...
    try {
      ...
      row.height = 130
      ws.addImage(imageId, {
        tl:  { col: (colIdx - 1) + 0.05, row: row.number - 1 + 0.05 },
  ...
  // ── Build sheets ─────────────────────────────────────────────
  const wsConf = makeTPSheet('✅ Confirmed Findings', '10B981', 'Flag Screenshot', 48)
  const wsRej  = makeTPSheet('❌ Rejected Findings',  'EF4444', 'Reviewer Note',   45)
  let confCount = 0, rejCount = 0

  for (const t of tenantResults) {
    const { property = '', tenantName = '', suite = '', confirmedFindings = [], rejectedFindings = [], annotations = [] } = t

    if (confirmedFindings.length === 0) {
      // All-clear row for this tenant on confirmed sheet
      addTPRow(wsConf, { property, tenant: tenantName, suite, missingDocument: 'None', comment: 'All AI findings were rejected or no findings generated.', severity: 'OK', checkType: '' }, 'OK', confCount, '')
      confCount++
    } else {
      for (const f of confirmedFindings) {
        addTPRow(wsConf, { property, tenant: tenantName, suite, ...f }, f.severity, confCount, '')
        confCount++
      }
    }

    for (const f of rejectedFindings) {
      addTPRow(wsRej, { property, tenant: tenantName, suite, ...f }, f.severity, rejCount, f.reviewerNote || '')
      rejCount++
    }

    // Flag-area annotations = things the AI MISSED → go on the Rejected sheet
    for (const ann of annotations) {
      const row = addTPRow(wsRej, {
        property, tenant: tenantName, suite,
        missingDocument: `🚩 Missed — ${ann.docName || 'Document'} · p.${ann.pageNum ?? '?'}`,
        comment: ann.comment || '', severity: 'HIGH', checkType: 'MANUAL FLAG'
      }, 'HIGH', rejCount, '')
      if (ann.cropDataUrl) embedShot(wsRej, row, ann.cropDataUrl, 8)  // col H = user-input
      rejCount++
    }
  }

  if (confCount === 0) wsConf.addRow(['No confirmed findings for this session', '', '', '', '', '', '', '', ''])
  if (rejCount  === 0) wsRej.addRow( ['No rejected findings for this session',  '', '', '', '', '', '', '', ''])

  // ── Summary sheet ─────────────────────────────────────────────
  const wsSumm = wb.addWorksheet('Summary', { tabColor: { argb: 'FF1D4ED8' } })
  wsSumm.columns = [{ width: 38 }, { width: 14 }]
  const totalTenants = tenantResults.length
  const totalConf    = tenantResults.reduce((s, t) => s + (t.confirmedFindings?.length || 0), 0)
  const totalRej     = tenantResults.reduce((s, t) => s + (t.rejectedFindings?.length || 0), 0)
  const totalAnno    = tenantResults.reduce((s, t) => s + (t.annotations?.length || 0), 0)
  const highConf     = tenantResults.reduce((s, t) => s + (t.confirmedFindings?.filter(f => f.severity === 'HIGH').length || 0), 0)
  const summRows = [
    ['TODD JR. — TARGET PRACTICE SESSION', ''],
    [`Reviewed by: ${reviewerName}  |  ${dateStr}`, ''],
    ['', ''],
    ['SESSION OVERVIEW', ''],
    ['Total Tenants', totalTenants],
    ['Confirmed Findings (AI was right)', totalConf],
    ['Rejected Findings (AI was wrong)', totalRej],
    ['Manual Flags — AI Missed (on Rejected sheet)', totalAnno],
    ['', ''],
    ['CONFIRMED BY SEVERITY', ''],
    ['High Severity Confirmed', highConf],
    ['Rules Generated from Session', juiceRules.length],
  ]
  summRows.forEach(([label, val], i) => {
    const row = wsSumm.addRow([label, val === '' ? '' : val])
    if (i === 0) { row.getCell(1).font = { bold: true, size: 13, name: 'Calibri', color: { argb: PALETTE.headerFont } }; row.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: PALETTE.headerBg } }; row.height = 24 }
    else if ([3, 9].includes(i)) { /* spacer */ }
    else if ([3, 9].includes(i + 0)) { /* section header */ }
    else if (label.toUpperCase() === label && label.trim()) { row.getCell(1).font = { bold: true, size: 10, name: 'Calibri', color: { argb: PALETTE.subHeader } } }
    else { row.getCell(1).font = { size: 10, name: 'Calibri' }; row.getCell(2).font = { bold: true, size: 10, name: 'Calibri' } }
  })

  // ── 📋 AUDIT REPORT sheet — ALWAYS included ─────────────────────────────
  // Three sections so the reviewer can spot any silent skip or duplicate:
  //   1. UPLOAD AUDIT — what came in (file count, folders, dedup events)
  //   2. OUTPUT AUDIT — what came out (which tenants appear in tenantResults)
  //   3. RECONCILIATION — uploaded vs produced, flagging skipped/duplicated
  // Tab is red if anything looks off, green if everything reconciles.
  {
    const d = uploadDiagnostics || {}

    // Build output side from tenantResults — which tenants actually got findings
    const outputTenantCounts = new Map()  // normalizedTenantName → count
    for (const tr of tenantResults) {
      const name = String(tr.tenantName || tr.tenant?.tenantName || '(unnamed)').trim()
      const key  = name.toLowerCase().normalize('NFC')
      outputTenantCounts.set(key, (outputTenantCounts.get(key) || 0) + 1)
    }

    // Build input side from uploadDiagnostics.finalTenantList (post-dedup)
    const inputTenantNames = new Map() // normalizedTenantName → display name
    for (const t of (d.finalTenantList || [])) {
      const key = String(t.tenantName || '').toLowerCase().normalize('NFC').trim()
      if (key) inputTenantNames.set(key, t.tenantName)
    }

    // Reconcile: find skipped (in input not output) and duplicated (in output > 1)
    const skippedTenants = []
    for (const [key, name] of inputTenantNames) {
      if (!outputTenantCounts.has(key)) skippedTenants.push(name)
    }
    const duplicatedTenants = []
    for (const [key, count] of outputTenantCounts) {
      if (count > 1) {
        const display = inputTenantNames.get(key) || key
        duplicatedTenants.push({ name: display, count })
      }
    }
    // orphan check is only meaningful when we have a manifest to compare against
    const haveManifestForOrphanCheck = inputTenantNames.size > 0
    const orphanTenants = []  // in output but not in input
    if (haveManifestForOrphanCheck) {
      for (const [key] of outputTenantCounts) {
        if (!inputTenantNames.has(key)) {
          orphanTenants.push(key)
        }
      }
    }

    // An ABSENT manifest is itself an issue — it means none of the checks below
    // could run, so the tab must not be painted green and must not claim a
    // "CLEAN RUN". Silence is not the same as a pass.
    const hasIssues =
      !uploadDiagnostics ||
      (d.duplicatesMerged?.length || 0) > 0 ||
      (d.emptyTenantsDropped?.length || 0) > 0 ||
      (d.fileCountDelta || 0) !== 0 ||
      skippedTenants.length > 0 ||
      duplicatedTenants.length > 0 ||
      orphanTenants.length > 0

    const tabArgb = hasIssues ? 'FFDC2626' : 'FF16A34A'
    const sheetName = hasIssues ? '⚠ Audit Report' : '✓ Audit Report'
    const wsDiag = wb.addWorksheet(sheetName, { tabColor: { argb: tabArgb } })
    wsDiag.columns = [{ width: 32 }, { width: 60 }, { width: 18 }]
    ...
    const issueFill  = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } }
    const okFill     = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDCFCE7' } }
    const sectionFill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E40AF' } }
    const sectionFont = { bold: true, color: { argb: 'FFFFFFFF' }, size: 11, name: 'Calibri' }

    // Title
    const titleRow = wsDiag.addRow([
      hasIssues ? '⚠ AUDIT REPORT — ISSUES DETECTED' : '✓ AUDIT REPORT — CLEAN RUN',
      '', ''
    ])
    wsDiag.mergeCells(titleRow.number, 1, titleRow.number, 3)
    ...
    // ─── Section A: Summary ───
    const summaryHdr = wsDiag.addRow(['SUMMARY', 'Value', ''])
    summaryHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    wsDiag.addRow(['Uploaded At', d.uploadedAt || '—', ''])
    wsDiag.addRow(['Files Uploaded', d.filesUploaded ?? '—', ''])
    wsDiag.addRow(['Folders Detected at Upload', d.foldersDetected ?? '—', ''])
    wsDiag.addRow(['Final Tenants After Dedup (INPUT)', (d.finalTenantList?.length || inputTenantNames.size || '—'), ''])
    wsDiag.addRow(['Tenants in Output (with findings)', outputTenantCounts.size, ''])
    wsDiag.addRow(['', '', ''])

    // ─── Section B: Reconciliation (the new core feature) ───
    const reconHdr = wsDiag.addRow(['INPUT vs OUTPUT RECONCILIATION', 'Detail', 'Count'])
    reconHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })

    if (skippedTenants.length === 0 && duplicatedTenants.length === 0 && orphanTenants.length === 0) {
      const r = wsDiag.addRow(['Reconciliation', 'Every uploaded tenant appears exactly once in output.', '✓'])
      r.eachCell(c => { c.fill = okFill })
    } else {
      if (skippedTenants.length > 0) {
        for (const name of skippedTenants) {
          const r = wsDiag.addRow([`⚠ SKIPPED — uploaded but no output`, name, 1])
          r.eachCell(c => { c.fill = issueFill })
        }
      }
      if (duplicatedTenants.length > 0) {
        for (const { name, count } of duplicatedTenants) {
          const r = wsDiag.addRow([`⚠ DUPLICATED — appears in output multiple times`, name, count])
          r.eachCell(c => { c.fill = issueFill })
        }
      }
      if (orphanTenants.length > 0) {
        for (const key of orphanTenants) {
          const r = wsDiag.addRow([`⚠ ORPHAN — in output but not in upload`, key, 1])
          r.eachCell(c => { c.fill = issueFill })
        }
      }
    }
    ...
    // ─── Section C: Upload-time duplicates merged ───
    const dupHdr = wsDiag.addRow(['UPLOAD DEDUP — DUPLICATE TENANTS MERGED', 'Detail', 'Files Added'])
    dupHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    if ((d.duplicatesMerged?.length || 0) === 0) {
      const r = wsDiag.addRow(['(none)', 'No duplicate-identity folders detected at upload.', ''])
      r.eachCell(c => { c.fill = okFill })
    } else {
      for (const m of d.duplicatesMerged) {
        const r = wsDiag.addRow([`"${m.duplicate}"`, `merged into "${m.mergedInto}"`, m.filesAdded])
        r.eachCell(c => { c.fill = issueFill })
      }
    }
    wsDiag.addRow(['', '', ''])

    // ─── Section D: Empty tenants dropped ───
    const emptyHdr = wsDiag.addRow(['EMPTY TENANTS DROPPED AT UPLOAD', 'Folder Name', ''])
    emptyHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    if ((d.emptyTenantsDropped?.length || 0) === 0) {
      const r = wsDiag.addRow(['(none)', 'Every uploaded tenant had files.', ''])
      r.eachCell(c => { c.fill = okFill })
    } else {
      for (const folder of d.emptyTenantsDropped) {
        const r = wsDiag.addRow(['Empty folder', folder, ''])
        r.eachCell(c => { c.fill = issueFill })
      }
    }
    wsDiag.addRow(['', '', ''])

    // ─── Section E: File count integrity ───
    const fcHdr = wsDiag.addRow(['FILE COUNT INTEGRITY', 'Status', 'Delta'])
    fcHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    if (!uploadDiagnostics) {
      // Do NOT paint this green. Absent diagnostics means the reconciliation
      // could not run at all, so a silently skipped or duplicated tenant would
      // pass through invisibly. That is a louder problem than a known delta.
      // Regression this guards: lawyer test 2026-08-03 — every report showed
      // this row, in green, while a tenant was in fact being skipped.
      const r = wsDiag.addRow([
        'File count check',
        '⚠ NOT VERIFIED — no upload manifest was available, so skipped/duplicated tenants could NOT be detected for this run',
        '—'
      ])
      r.eachCell(c => { c.fill = issueFill })
    } else if ((d.fileCountDelta || 0) === 0) {
      const r = wsDiag.addRow(['Files uploaded vs files in final tenants', 'Match — no files lost', 0])
      r.eachCell(c => { c.fill = okFill })
    } else {
      const r = wsDiag.addRow(['Files uploaded vs files in final tenants', `Mismatch — ${d.fileCountDelta} files unaccounted for`, d.fileCountDelta])
      r.eachCell(c => { c.fill = issueFill })
    }
    wsDiag.addRow(['', '', ''])

    // ─── Section F: Full input manifest (post-dedup) ───
    const inputHdr = wsDiag.addRow(['UPLOADED TENANTS (INPUT)', 'Property / Suite', 'File Count'])
    inputHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    if ((d.finalTenantList?.length || 0) === 0) {
      wsDiag.addRow(['(no upload manifest available)', '', ''])
    } else {
      for (const t of d.finalTenantList) {
        wsDiag.addRow([t.tenantName, `${t.property || '—'} / ${t.suite || '—'}`, t.fileCount])
      }
    }
    wsDiag.addRow(['', '', ''])

    // ─── Section G: Output tenants (with findings count) ───
    const outHdr = wsDiag.addRow(['OUTPUT TENANTS (RESULTS)', 'Findings (Confirmed + Rejected)', 'Status'])
    outHdr.eachCell(c => { c.fill = sectionFill; c.font = sectionFont })
    if (tenantResults.length === 0) {
      wsDiag.addRow(['(no results)', '', ''])
    } else {
      // CRITICAL: Only flag tenants as ORPHAN when we ACTUALLY have an input
      // manifest to compare against. Otherwise (older sessions, no upload
      // diagnostics captured), every output row would falsely show ORPHAN.
      const haveManifest = inputTenantNames.size > 0
      for (const tr of tenantResults) {
        const name = String(tr.tenantName || tr.tenant?.tenantName || '(unnamed)').trim()
        const conf = (tr.confirmedFindings?.length || 0)
        const rej  = (tr.rejectedFindings?.length || 0)
        const total = conf + rej
        const key = name.toLowerCase().normalize('NFC')
        const inInput = inputTenantNames.has(key)
        const count = outputTenantCounts.get(key) || 1
        let status = '✓'
        if (haveManifest && !inInput) status = '⚠ ORPHAN'
        else if (count > 1) status = `⚠ DUPLICATED (${count}×)`
        else if (!haveManifest) status = '(no manifest)'
        const r = wsDiag.addRow([name, `${total} (${conf} kept / ${rej} rejected)`, status])
        if (status.startsWith('⚠')) r.eachCell(c => { c.fill = issueFill })
      }
    }
  }

  if (outputPath) await wb.xlsx.writeFile(outputPath)
  const buffer = await wb.xlsx.writeBuffer()
  return Buffer.from(buffer)
}
