...
export async function generateTargetPracticeSessionExcel(opts, outputPath) {
  ...
  for (const t of tenantResults) {
    ...
    if (confirmedFindings.length === 0 && annotations.length === 0) {
      ...
      for (const ann of annotations) {
        const row = addTPRow(wsConf, {
          property, tenant: tenantName, suite,
          missingDocument: `Flag — ${ann.docName || 'Document'} · p.${ann.pageNum ?? '?'}`,
          comment: ann.comment || '', severity: 'NOTE', checkType: 'MANUAL FLAG'
        }, 'NOTE', confCount, '')
        if (ann.cropDataUrl) embedShot(wsConf, row, ann.cropDataUrl, 8)
        confCount++
      }
    }
  ...
  const summRows = [
    ...
    ['Rejected Findings (AI was wrong / partial)', totalRej],
    ['Manual Flags / Annotations', totalAnno],
    ['', ''],
