...
const TODD_AUDIT_JSON_SCHEMA = {
  ...
  properties: {
    ...
    chainWalk: {
      type: 'array',
      description: 'One row per term-bearing document. coversFrom/coversThrough are what the post-merge pass stitches across batches — every property must stay listed in `required` for strict mode.',
      items: {
        type: 'object',
        additionalProperties: false,
        properties: {
          doc: { type: 'string' },
          coversFrom: { type: ['string', 'null'] },
          coversThrough: { type: ['string', 'null'] }
        },
        required: ['doc', 'coversFrom', 'coversThrough']
      }
    },
    chainGaps: {
      type: 'array',
      items: {
        ...
        properties: {
          between: { type: 'string' },
          and: { type: 'string' },
          uncoveredDays: { type: 'number' }
        },
        required: ['between', 'and', 'uncoveredDays']
      }
    },
    findings: {
      type: 'array',
  ...
  required: ['tenantNameInDocuments', 'mostRecentDocumentDate', 'leaseExpirationDate', 'chainWalk', 'chainGaps', 'findings', 'allClear']
}
...
/** Env or openai.key file (see loadOpenAiKeyFile). */
export function isOpenAiKeyConfigured() {
  return !!(process.env.OPENAI_API_KEY?.trim() || _fileOpenAiKey)
}

/** For /api/health: env | openai.key | none */
...
/**
 * Priority: explicit per-request override (rare) > openai.key file > OPENAI_API_KEY env
 */
function resolveOpenAiOverride(explicitFromCaller) {
  ...
  if (ex) return { key: ex, source: 'explicit' }
  if (_fileOpenAiKey) return { key: _fileOpenAiKey, source: 'openai.key' }
  return { key: '', source: 'server_env' }
}

/** Default 20m — native-PDF folder audits can exceed SDK default (~10m). Override with OPENAI_TIMEOUT_MS. */
function openAiClientOptions(apiKey) {
  const parsed = Number(process.env.OPENAI_TIMEOUT_MS)
  const timeout = Number.isFinite(parsed) && parsed > 0 ? parsed : 1_200_000
...
function getOpenAIClient(overrideKey) {
  const trimmed =
    overrideKey !== undefined && overrideKey !== null && String(overrideKey).trim()
      ...
      : ''
  if (trimmed) return new OpenAI(openAiClientOptions(trimmed))
  const k = process.env.OPENAI_API_KEY?.trim()
  if (!k) return null
  if (!_openaiClient) _openaiClient = new OpenAI(openAiClientOptions(k))
  return _openaiClient
}

/** Fire periodic progress while awaiting a long OpenAI HTTP call (SSE would otherwise look stuck). */
async function withOpenAiWaitProgress(onProgress, basePercent, label, fn) {
  if (typeof onProgress !== 'function') return fn()
  const pct = Math.min(94, Math.max(0, Number(basePercent) || 35))
  const prefix = String(label || 'OpenAI').replace(/\.\.\.$/, '').trim()
  onProgress({
    percent: pct,
    message: `${prefix} — submitted to OpenAI; waiting for response (often several minutes with many PDFs).`
  })
  const t0 = Date.now()
  const id = setInterval(() => {
    ...
    onProgress({
      percent: pct,
      message: `${prefix} — still waiting on OpenAI (${sec}s elapsed)…`
    })
  }, 15000)
  try {
    return await fn()
...
function normalizeResult(parsed, tenant) {
  ...
  /* The strict schema REQUIRES chainWalk/chainGaps, so carry them through rather
     than asking for diagnostics and dropping them. mergeResults reconciles the
     walk across batches and cannot do that from keys this function deleted. */
  const out = {
  ...
  if (Array.isArray(parsed?.chainWalk)) out.chainWalk = parsed.chainWalk
  if (Array.isArray(parsed?.chainGaps)) out.chainGaps = parsed.chainGaps
  return out
}
...
• chainWalk: one row per term-bearing document, with BOTH "coversFrom" (the date its term or extended period begins) and "coversThrough" (the date it runs to), in YYYY-MM-DD. Write null only for a date the file never establishes. Emit these rows even when your walk finds no gap — when a folder is split into batches they are stitched together afterwards to find holes that straddle a batch boundary, which no single batch can see.
• chainGaps: every uncovered interval your own walk found, including ones you expect to drop.
...
/**
 * Parse the retry-after delay from a 429 error message.
 * OpenAI embeds "try again in Xs" in the message.
 */
function parse429WaitMs(err) {
  const msg = String(err?.message || err?.error?.message || '')
  const m = msg.match(/try again in\s+([\d.]+)\s*s/i)
  if (m) return Math.ceil(parseFloat(m[1]) * 1000) + 1000 // add 1s buffer
  return 15000 // default 15s if we can't parse
}

/**
 * Wrap an OpenAI API call with up to 3 automatic retries on 429 rate-limit errors.
 * Waits the time suggested in the error message before retrying.
 */
async function withOpenAi429Retry(fn, label = '') {
  const MAX_RETRIES = 3
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const status = err?.status ?? err?.statusCode
      const is429 = status === 429 || /rate.?limit|429/i.test(String(err?.message || ''))
      if (!is429 || attempt === MAX_RETRIES) throw err
      const waitMs = parse429WaitMs(err)
      console.warn(`[openai] 429 rate limit${label ? ' (' + label + ')' : ''} — waiting ${Math.round(waitMs / 1000)}s then retrying (attempt ${attempt + 1}/${MAX_RETRIES})`)
...
⚠️ CRITICAL FOR THE LEASE CURRENCY CHAIN WALK: emit a chainWalk row with BOTH "coversFrom" and "coversThrough" for every term-bearing document in this batch, even when your own walk shows no gap. The merge stitches all ${batchInfo.totalBatches} batches' rows together to find uncovered intervals that straddle a batch boundary — holes no single batch, including this one, can see. Omitted rows make those holes invisible.
${'═'.repeat(70)}`
  }
...
async function runOpenAiPdfBatch(
  ...
  const resp = await withOpenAiWaitProgress(
    onProgress,
    progressPercent,
    progressLabel,
    () =>
      withOpenAi429Retry(
        () => openaiResponsesCreate(
          client,
          {
            ...
            max_output_tokens: options.cheapMode ? 2000 : 3000
          },
          true
        ),
        `batch PDF — ${pdfBatch.map(p => p.filename).join(', ')}`
      )
  )
...
async function runOpenAiTextOnly(
  client,
  model,
  tenant,
  docs,
...
export async function openaiAnalyzeTenant(tenant, files, onProgress = () => {}, options = {}) {
  ...
  if (!overrideKey && !keyFromEnv) {
    throw new Error(
      'No OpenAI API key. Create openai.key in the project folder (one line, sk-…), or set OPENAI_API_KEY in .env or Railway Variables.'
    )
  }
  const client = getOpenAIClient(overrideKey || undefined)
  if (!client) {
    throw new Error(
      'No OpenAI API key. Add openai.key or OPENAI_API_KEY in .env (or Railway Variables).'
    )
  }
  ...
  // Read all files in parallel — no sequential blocking per-file
    ...
    if (ext === '.pdf') {
      try {
        ...
        if (sizeBytes <= PDF_MAX_BYTES) {
          ...
          // Skip extractText page-count pre-scan — cosmetic only, saves ~0.5s per PDF
          pdfCandidates.push({
            filename: file.originalName,
            diskPath: file.diskPath,
            base64,
            pageCount: null,  // filled in from API response if needed
            sizeBytes
          })
        } else {
  ...
  // Cheap (Dumb) mode: skip vision tokens entirely — extract text and send as text blocks.
  // Vision tokens cost ~1700/page; text tokens cost ~1/token. For gpt-4o-mini this prevents
  // TPM rate-limit 429s on normal-sized folders.
  if (options.cheapMode && pdfCandidates.length > 0) {
    onProgress({ percent: 20, message: 'OpenAI (dumb mode): extracting text from PDFs...' })
    const { extractText: et } = await import('./parser.js')
    const flatDocs = []
    await Promise.all(pdfCandidates.map(async p => {
      try {
        const doc = await et(p.diskPath, p.filename)
        flatDocs.push({ name: p.filename, text: doc.text || '', pages: doc.pageCount || 0 })
      } catch {
        flatDocs.push({ name: p.filename, text: '[extraction error]', pages: 0 })
      }
    }))
    for (const d of textDocs) {
      flatDocs.push({ name: d.filename, text: d.text || '', pages: d.pageCount || 0 })
    }
    onProgress({ percent: 40, message: 'OpenAI (dumb mode): text-only analysis — no vision tokens...' })
    const out = await runOpenAiTextOnly(client, model, tenant, flatDocs, pageGapFindings, options, onProgress, 42, 'OpenAI: dumb mode text analysis')
    ...
    if (options.includeDebug) {
      out._openaiDebug = {
        ...
        cheapMode: true,
        openaiKeySource,
        analysisPath: 'dumb_mode_text_only',
        explanation: 'Cheap/Dumb mode: PDFs extracted to text locally, sent as text blocks. No vision/image tokens used — dramatically lower TPM cost.',
        tenantFilesTotal: files.length,
        pdfCountExtractedAsText: pdfCandidates.length,
      }
    }
    return out
  }
  ...
  if (pdfCandidates.length === 0) {
    ...
    const out = await runOpenAiTextOnly(
      client,
      model,
      tenant,
      flatDocs,
        ...
        for (const single of batch) {
          ...
          const oneInfo =
            totalBatch > 1
              ...
              : null
          const onePct = pct
          const oneLabel = `OpenAI: 413 recovery ${si}/${batch.length} — ${single.filename}`
          const result = await runOpenAiPdfBatch(
            client,
            model,
            tenant,
  ...
  const out = normalizeResult(
    {
      ...
      // Reconciled by mergeResults across every batch — including any
      // cross-batch term-chain gap findings it appended.
      chainWalk: merged.chainWalk,
      chainGaps: merged.chainGaps,
