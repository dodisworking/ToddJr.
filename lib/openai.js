...
export async function openaiAnalyzeTenant(tenant, files, onProgress = () => {}, options = {}) {
  ...
  if (pdfCandidates.length === 0) {
    ...
    if (options.includeDebug) {
      out._openaiDebug = {
        ...
        explanation:
          'No PDFs under 32MB were queued as native files. Each document was read locally; extracted text was sent in one user message (no input_file PDF parts).',
        tenantFilesTotal: files.length,
        nativePdfCount: 0,
  ...
  if (options.includeDebug) {
    out._openaiDebug = {
      ...
      explanation:
        'Each API call sends: system instructions + user content with one input_text preamble, then one input_file per PDF in that batch (data:application/pdf;base64,...), then input_text blocks for every non-PDF / text-only doc (repeated in each batch, same as Claude-side text attachments).',
      tenantFilesTotal: files.length,
