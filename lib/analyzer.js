...
const BATCH_MAX_BASE64   = 25 * 1024 * 1024  // 25MB per batch — covers ~18.75MB raw PDFs after base64 overhead (×1.33)
...
const SPLIT_TARGET_BYTES = 15 * 1024 * 1024
 ...
 */
export function buildBatches(candidates, maxBytes) {
...
async function _orchestrateAnalysis(tenant, files, onProgress, analyzeFolder_fn, options = {}) {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    // Build text extracts of all PDFs NOT in this batch so Claude can reason
    // across the full folder even when documents are split across batches.
    // Only include PDFs where text was actually extractable (scanned-only PDFs
    // will have empty text — no point passing empty strings).
    const otherBatchTexts = totalBatch > 1
      ? pdfCandidates
          .filter(p => !batch.some(bp => bp.filename === p.filename))
          .filter(p => p.extractedText && p.extractedText.trim().length > 50)
          .map(p => ({ filename: p.filename, text: p.extractedText, pageCount: p.pageCount, isFromOtherBatch: true }))
      : []
