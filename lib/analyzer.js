...
import { analyzeFolder, gymAnalyzeFolder, beefedUpAnalyzeFolder, doubleCheckFolder, synthesizeAcrossBatches, filterFindingsForRelevance, verifyExecutionFindings } from './claude.js'
...
export async function beefedUpAnalyzeTenant(tenant, files, onProgress, learnings = [], options = {}) {
  ...
  if (verifiedFindings && verifiedFindings.length > 0 && activeLearnings.length > 0) {
    try {
      onProgress({ percent: 97, message: 'Relevance check...' })
      const filtered = await filterFindingsForRelevance(verifiedFindings, activeLearnings, tenant.tenantName)
      return { ...raw, findings: filtered, allClear: filtered.length === 0 }
    } catch (err) {
