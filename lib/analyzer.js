...
const OTHER_BATCH_TEXT_CAP = 8000            // max chars of extracted text from each out-of-batch PDF (~5-6 pages of content, covers most cross-references)
...
async function _orchestrateAnalysis(tenant, files, onProgress, analyzeFolder_fn, options = {}) {
  ...
  try {
    const synthesized = await synthesizeAcrossBatches(
      ...
      options,
      pdfCandidates   // ← full text of every document for complete re-analysis
    )
    onProgress({ percent: 100, message: 'Complete' })
    return synthesized
  } catch (err) {
