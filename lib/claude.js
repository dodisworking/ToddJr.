...
// ── API key rotation — failover on 429 rate-limit ──────────
// Set ANTHROPIC_API_KEY (primary) and optionally ANTHROPIC_API_KEY_2 (backup)
// in your Railway (or .env) variables.  On a 429 the call is retried once with
// the next key so rate-limits on the primary never stall a request.
const _KEYS = [
  process.env.ANTHROPIC_API_KEY,
  process.env.ANTHROPIC_API_KEY_2
].filter(Boolean)
// _keyIdx advances on every API call so concurrent requests naturally interleave
// across both keys — doubling effective rate-limit capacity when two keys exist.
let _keyIdx = 0

const client = new Anthropic({ apiKey: _KEYS[0] })  // legacy ref for streaming

/**
 * Pick next key in round-robin order and return a fresh Anthropic client.
 * With two keys: call 1 → key0, call 2 → key1, call 3 → key0, …
 */
function nextClient() {
  const key = _KEYS[_keyIdx % _KEYS.length]
  if (_KEYS.length > 1) _keyIdx++
  return new Anthropic({ apiKey: key })
}

/**
 * messages.create — proactive round-robin + 429 immediate failover.
 */
async function createMessageWithFailover(params) {
  const idx = _keyIdx % _KEYS.length
  const c   = nextClient()
  try {
    return await c.messages.create(params)
  } catch (err) {
    if (err?.status === 429 && _KEYS.length > 1) {
      const fallbackIdx = (idx + 1) % _KEYS.length
      console.warn(`[claude] 429 on key ${idx} → immediate fallback to key ${fallbackIdx}`)
      return await new Anthropic({ apiKey: _KEYS[fallbackIdx] }).messages.create(params)
    }
    throw err
  }
}
...
/**
 * beta.messages.create — proactive round-robin + 429 immediate failover.
 * Concurrent tenant analyses automatically spread across both API keys.
 */
async function createBetaMessageWithFailover(params) {
  ...
  try {
    ...
    if (err?.status === 429 && _KEYS.length > 1) {
      const fallbackIdx = (idx + 1) % _KEYS.length
      console.warn(`[claude] 429 on key ${idx} → immediate fallback to key ${fallbackIdx}`)
      return await new Anthropic({ apiKey: _KEYS[fallbackIdx] }).beta.messages.create(params)
    }
    throw err
  }
}
...
export async function synthesizeDeepLearning({ tenantFeedbacks = [] }) {
  ...
  const tenantBlocks = tenantFeedbacks.map((fb, i) => {
    const rejected   = fb.rejectedFindings?.length > 0
      ? fb.rejectedFindings.map((f, j) =>
          `  ${j + 1}. [${f.checkType}] "${f.missingDocument}"\n     Evidence: ${f.evidence || 'none'}\n     Reviewer note: ${f.reviewerNote || 'wrong'}`
        ).join('\n\n')
      : '  None — all confirmed'
    const confirmed  = fb.confirmedFindings?.length > 0
      ? fb.confirmedFindings.map(f => `  • [${f.checkType}] "${f.missingDocument}"`).join('\n')
      : '  None'
    const missed     = fb.annotations?.length > 0
      ? fb.annotations.map((a, j) => `  ${j + 1}. "${a.comment}" on ${a.docName || 'unknown'} p.${a.pageNum || '?'}`).join('\n')
      : '  None'
    return `── TENANT ${i + 1}: ${fb.tenantName || 'Unknown'} ──
REJECTED (wrong findings):
${rejected}

CONFIRMED (correct findings):
${confirmed}

MISSED (reviewer annotated items AI missed):
${missed}`
  }).join('\n\n')

  const prompt = `You are the Deep Juice synthesis engine for Todd Jr., a real estate document review AI.

An expert paralegal has finished reviewing ${tenantFeedbacks.length} tenants in a single Rapid Batch session. Below is the FULL session feedback across all tenants.

${tenantBlocks}

YOUR JOB:
Analyze ALL the above feedback together. Your goal is to find CROSS-TENANT PATTERNS — recurring mistakes, systematic blind spots, and confirmed strengths — that a single-tenant synthesis would miss.

For each rejected finding type, determine:
1. Is this a systematic false-positive pattern, or a one-off mistake?
2. What evidence patterns should trigger a flag vs. what should suppress it?
3. Is there a conditional rule that captures the nuance without suppressing real issues?

For each missed annotation, determine:
1. Is this a systematic miss across multiple tenants?
2. What evidence pattern should Claude look for?

Produce a comprehensive rule set covering ALL patterns found across the full session. This set should be more powerful and precise than what per-tenant synthesis would generate.

Rules must be:
- Evidence-pattern specific, NOT tenant-name specific
- Conditional where evidence patterns differ
- Precise enough to avoid suppressing real issues
...
Return ONLY valid JSON (no markdown, no prose, no code fences):
{
  "rules": [
    {
      ...
      "checkType": "EXECUTION",
      "rule": "Clear actionable instruction for the next analysis",
      "triggeredBy": "Pattern seen across N tenants in this session",
      "confidence": 2
    }
  ],
  "summary": "One sentence: key patterns found across the ${tenantFeedbacks.length}-tenant session"
}`
  ...
  const response = await createMessageWithFailover({
    ...
    max_tokens: 4096,
    temperature: 0.1,
    system: [{ type: 'text', text: 'You are the Deep Juice synthesis engine for a real estate document review AI. Return only valid JSON.' }],
    messages: [{ role: 'user', content: [{ type: 'text', text: prompt }] }]
  })
