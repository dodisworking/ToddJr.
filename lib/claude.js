 ...
 */
/**
 * Juice rules block only — no base system prompt prepended.
 * Used when combining with global learnings so both sections coexist.
 */
function buildJuiceRulesBlock(juiceRules) {
  if (!juiceRules || juiceRules.length === 0) return ''
...
${'═'.repeat(70)}
An expert paralegal reviewed prior tenant analyses in this session and generated these correction rules. Apply them in addition to all rules above.
...
function buildJuiceSystemPrompt(juiceRules) {
  ...
  return `${SYSTEM_PROMPT}\n\n${buildJuiceRulesBlock(juiceRules)}`
}
...
async function gymCallWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  ...
  try {
    ...
    // Build system prompt: global trained rules always come first as the permanent
    // foundation, then session juice rules stack on top as additional refinements.
    // Never let juice rules replace global learnings — they must coexist.
    const systemPrompt = (() => {
      const hasGlobal = options.activeLearnings?.length > 0
      const hasJuice  = options.juiceRules?.length > 0
      if (hasGlobal && hasJuice) {
        // Both: global rules as foundation + juice rules as session layer on top
        return buildBeefedUpSystemPrompt(options.activeLearnings) + '\n\n' +
          buildJuiceRulesBlock(options.juiceRules)
      }
      if (hasJuice)  return buildJuiceSystemPrompt(options.juiceRules)
      if (hasGlobal) return buildBeefedUpSystemPrompt(options.activeLearnings)
      return SYSTEM_PROMPT
    })()
...
2. TRAINING RULE CHECK: Does the finding match any validated training rule below? Apply each rule as written — if the finding pattern matches → DROP.
...
KEEP only findings that describe a real, unresolved issue requiring reviewer attention.
...
Apply the three checks above. Respond with KEEP or DROP for every finding [0] through [${findings.length - 1}]. Do not skip any index.`
