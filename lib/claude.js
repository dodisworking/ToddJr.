...
export async function synthesizeDeepLearning({ tenantFeedbacks = [] }) {
  ...
  // ── Pre-aggregate patterns across ALL tenants (scales to 50+) ──────────
  // rejectedByType: how many tenants rejected each checkType, with up to 3 examples
  const rejectedByType  = {}   // checkType → { count, examples[] }
  // confirmedByType: how many times each checkType was confirmed correct
  const confirmedByType = {}   // checkType → count
  // checkTypes that appeared in BOTH confirmed and rejected (nuanced rules needed)
  const mixedTypes      = new Set()
  // all missed items (reviewer annotations = things AI should have caught)
  ...
  for (const fb of tenantFeedbacks) {
    for (const f of (fb.rejectedFindings || [])) {
      ...
      totalRejected++
      if (rejectedByType[k].examples.length < 3) {
        rejectedByType[k].examples.push({
          doc:      f.missingDocument || '—',
          evidence: (f.evidence || '').slice(0, 200),
          note:     f.reviewerNote   || 'wrong',
          tenant:   fb.tenantName    || 'Unknown'
        })
      }
    }
    for (const f of (fb.confirmedFindings || [])) {
      const k = f.checkType || 'UNKNOWN'
      confirmedByType[k] = (confirmedByType[k] || 0) + 1
      totalConfirmed++
    }
    for (const a of (fb.annotations || [])) {
      allAnnotations.push({
        comment: a.comment || '—',
        docName: a.docName || 'unknown',
        pageNum: a.pageNum || '?',
        tenant:  fb.tenantName || 'Unknown'
      })
    }
  ...
  // Flag checkTypes that were both confirmed and rejected (conditional rules needed)
  for (const k of Object.keys(rejectedByType)) {
    if (confirmedByType[k]) mixedTypes.add(k)
  }

  // ── Build aggregated prompt sections ───────────────────────────────────

  // SECTION 1: Rejection patterns (sorted by frequency, most common first)
  const rejectionLines = Object.entries(rejectedByType)
    .sort((a, b) => b[1].count - a[1].count)
    .map(([type, d]) => {
      const pct      = Math.round((d.count / n) * 100)
      const mixed    = mixedTypes.has(type) ? ` ⚠️ ALSO CONFIRMED ${confirmedByType[type]}x — conditional rule needed` : ''
      const examples = d.examples.map(e =>
        `    • "${e.doc}" [${e.tenant}] — reviewer: "${e.note}"${e.evidence ? `\n      evidence snippet: "${e.evidence}"` : ''}`
      ).join('\n')
      return `[${type}] rejected ${d.count}/${n} tenants (${pct}%)${mixed}\n${examples}`
    }).join('\n\n')

  // SECTION 2: Confirmed strengths (checkTypes that were consistently correct)
  const confirmLines = Object.entries(confirmedByType)
    .filter(([k]) => !rejectedByType[k])  // only pure confirmations, no mixed
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([type, count]) => `  [${type}] confirmed correct ${count} times across ${n} tenants`)
    .join('\n')

  // SECTION 3: Missed items (things Claude should have flagged but didn't)
  const missedLines = allAnnotations.length > 0
    ? allAnnotations.map(a =>
        `  • [${a.tenant}] "${a.comment}" — doc: ${a.docName} p.${a.pageNum}`
      ).join('\n')
    : '  None — no systematic misses found'

  const prompt = `You are the Deep Juice synthesis engine for Todd Jr., a real estate document review AI.

A paralegal expert has reviewed ${n} tenants in a Rapid Batch session.
Session stats: ${totalConfirmed} confirmed correct · ${totalRejected} wrong flags · ${allAnnotations.length} missed items

════════════════════════════════════════
SECTION A — WRONG FLAGS (what to STOP doing)
These checkTypes had findings the reviewer rejected as incorrect.
Sorted by frequency. ⚠️ = same type was also confirmed correct → write a CONDITIONAL rule.
════════════════════════════════════════
${rejectionLines || '  None'}

════════════════════════════════════════
SECTION B — CONFIRMED STRENGTHS (keep doing these)
CheckTypes that were consistently flagged correctly.
════════════════════════════════════════
${confirmLines || '  None'}

════════════════════════════════════════
SECTION C — SYSTEMATIC MISSES (what to DO MORE of)
Items the reviewer manually annotated that the AI missed entirely.
These represent blind spots — write PRIORITIZE_CHECK rules for patterns here.
════════════════════════════════════════
${missedLines}
...
1. AVOID_FALSE_POSITIVE — for Section A rejections
   "When [evidence pattern], do NOT flag [checkType] as missing"
   For ⚠️ mixed types: write a precise condition so real issues still get flagged

2. PRIORITIZE_CHECK — for Section C misses
   "Look harder for [item type] — specifically check [where/what evidence to look for]"
   These add vigilance, not suppression

3. CONDITIONAL_FLAG — for ⚠️ mixed types only
   "Flag [checkType] ONLY when [condition A] — suppress when [condition B]"

Confidence scoring:
  3 = pattern in >50% of tenants or multiple annotations of same type
  2 = pattern in 20–50% of tenants
  1 = single occurrence (still worth a rule if reviewer note was explicit)
...
Return ONLY valid JSON (no markdown, no code fences):
{
  "rules": [
    {
      ...
      "type": "AVOID_FALSE_POSITIVE",
      "checkType": "EXECUTION",
      "rule": "Precise actionable instruction referencing the evidence pattern",
      "triggeredBy": "Rejected ${Object.values(rejectedByType)[0]?.count || 'N'}/${n} tenants — [brief reason]",
      "confidence": 2
    }
  ],
  "summary": "One sentence: top patterns found across the ${n}-tenant session"
}`
  ...
  const response = await createMessageWithFailover({
    ...
    max_tokens: 8192,
    temperature: 0.1,
    system: [{ type: 'text', text: 'You are the Deep Juice synthesis engine for a real estate document review AI. Return only valid JSON. No markdown, no prose outside the JSON object.' }],
    messages:  [{ role: 'user', content: [{ type: 'text', text: prompt }] }]
  })
  ...
  const jsonStart = cleaned.indexOf('{')
  if (jsonStart > 0) cleaned = cleaned.substring(jsonStart)
  // Trim any trailing content after the final closing brace (truncation safety)
