...
export async function filterFindingsForRelevance(findings, activeLearnings, tenantName) {
  ...
  const findingsBlock = findings.map((f, i) => {
    // Include full comment + evidence so the filter can detect self-contradictions
    const main = (f.missingDocument || f.comment || 'No description').substring(0, 600)
...
1. SELF-CONTRADICTION CHECK (highest priority): Does the finding's own text already dismiss or explain away the issue? Look for phrases like "this is acceptable", "not a mismatch", "executed version is operative", "not a deficiency", "sufficient", "this is acceptable shorthand", "no missing document", "fully executed" anywhere in the finding description. If the AI's own text resolves the concern → DROP immediately. This is the most common false-positive pattern.
