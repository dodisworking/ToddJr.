...
import { formatChainCoverageForPrompt, MIN_REPORTABLE_GAP_DAYS } from './chain.js'
...
// ═══════════════════════════════════════════════════════════════════════════
// SELF-SUPPRESSION FILTER — ALWAYS APPLIED
// ═══════════════════════════════════════════════════════════════════════════
// Strips findings whose own text says they should be suppressed. The Todd
// filter (filterFindingsForRelevance) has this list too, but Todd only runs
// in TP3 mode. In TP2/gym mode, the generator's output goes straight to the
// reviewer — so we need an always-on version. Catches the lawyer-reported
// "the comment says SUPPRESSED but the finding is emitted anyway" bug.
// ═══════════════════════════════════════════════════════════════════════════

const SELF_SUPPRESSION_PHRASES = [
  // direct self-suppression
  'WITHDRAWING', 'WITHDRAWN', 'WITHDRAW THIS FINDING', 'I AM WITHDRAWING',
  'RETRACT', 'RETRACTING',
  'FINDING DROPPED', 'DROPPING THIS FINDING', 'DROP THIS FINDING', 'SHOULD BE DROPPED',
  'SUPPRESSED', 'BEING SUPPRESSED', 'IS SUPPRESSED', 'MUST BE SUPPRESSED', 'SHOULD BE SUPPRESSED',
  // explicit FP admission
  'FALSE POSITIVE',
  'NO FINDING IS WARRANTED', 'NO FINDING WARRANTED', 'NO FINDING IS GENERATED', 'NO FINDING IS OUTPUT',
  'NOT A VALID FINDING', 'THIS FINDING IS THEREFORE NOT VALID', 'THIS FINDING IS NOT VALID',
  'NOT FLAGGED AS A FINDING', 'IS NOT BEING FLAGGED', 'WOULD NOT BE FLAGGED', 'SHOULD NOT BE FLAGGED',
  'THIS IS NOT A FINDING', 'NOT ACTIONABLE', 'DOES NOT NEED TO BE NOTED', 'NEED NOT BE NOTED',
  // operative document is fully signed
  'IS FULLY EXECUTED', 'ARE FULLY EXECUTED', 'EXECUTION IS COMPLETE', 'NO EXECUTION DEFICIENCY',
  'NO EXECUTION FINDING WARRANTED', 'NO EXECUTION DEFECT EXISTS',
  'BOTH PARTIES HAVE SIGNED', 'ALL PARTIES HAVE SIGNED', 'BOTH PARTIES SIGNED', 'ALL REQUIRED PARTIES SIGNED',
  // "for training / awareness only"
  'FOR TRAINING TRANSPARENCY ONLY', 'FOR TRAINING PURPOSES', 'TRAINING TRANSPARENCY',
  'FOR REVIEWER AWARENESS', 'FOR AWARENESS ONLY',
  // self-check
  'SELF-RESOLVING', 'IS SELF-RESOLVING', 'SELF-CHECK — NO FINDING', 'THIS IS A SELF-CHECK',
  // "no missing document / executed version exists"
  'NO MISSING DOCUMENT', 'EXECUTED VERSION IS OPERATIVE', 'EXECUTED VERSION EXISTS', 'SIGNED VERSION EXISTS',
  'STANDALONE GUARANTY FILE IS EXECUTED', 'STANDALONE COPY IS SIGNED', 'OTHER COPY IS SIGNED',
  'ANOTHER COPY IS FULLY EXECUTED',
  // text-flow self-resolved page-gap
  'TEXT FLOWS CONTINUOUSLY', 'NO CONTENT GAP', 'NO CONTENT SCAR',
]

const SELF_SUPPRESSION_PATTERNS = SELF_SUPPRESSION_PHRASES.map(p => p.toUpperCase())

function findingTextForCheck(f) {
  return [
    f.missingDocument || '',
    f.comment || '',
    f.evidence || '',
...
/**
 * Drops findings whose own text admits the finding is invalid / suppressed /
 * already resolved. Returns the surviving findings. Logs each drop with the
 * phrase that triggered it so we can debug.
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, source }  for logging only
 * @returns {Array<object>}  filtered findings
 */
export function dropSelfSuppressedFindings(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag = opts.tenantName ? `[self-suppress ${opts.source || ''} ${opts.tenantName}]` : `[self-suppress]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null
  const kept = []
  for (const f of findings) {
    const txt = findingTextForCheck(f)
    const hit = SELF_SUPPRESSION_PATTERNS.find(p => txt.includes(p))
    if (hit) {
      const preview = (f.missingDocument || f.comment || '').slice(0, 120)
      console.log(`${tag} DROP — phrase "${hit}" — ${preview}`)
      if (onDrop) try { onDrop(f, `phrase:"${hit}"`) } catch {}
      continue
    }
    kept.push(f)
  }
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after self-suppression check`)
  }
  return kept
}

// Entity-form suffixes and filler words that carry no identifying signal.
const ENTITY_NOISE = new Set([
  'llc','l.l.c','lp','l.p','llp','inc','incorporated','corp','corporation','co',
  'company','pc','p.c','pa','p.a','ltd','limited','na','n.a','plc','trust',
  'partners','partnership','holdings','holding','group','associates','association',
  'the','of','and','a','an','delaware','texas','national','banking','bank'
])

/** Distinctive lowercase tokens from an entity or folder label. */
function entityTokens(name) {
  return String(name || '')
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter(t => t.length >= 3 && !ENTITY_NOISE.has(t))
}

/**
 * Drops NAME_MISMATCH findings that are really comparing an ADMINISTRATIVE
 * LABEL (the folder name, a tab name, or a PDF filename) against the legal
 * entity name inside the document.
 *
 * CHECK 10 already tells the model "Folder names and file names are assigned by
 * the client and are IRRELEVANT to this check. Never compare a folder name or
 * file name to a legal entity name" — and the model did it anyway on the
 * 2026-08-03 lawyer test (FP83-16: folder "Financial Synergies Wealth" vs the
 * lease's "Financial Synergies Asset Management, Inc."). A prompt instruction
 * that is demonstrably ignored has to be enforced in code.
 *
 * Two independent triggers, either one drops the finding:
 *   (a) the finding text itself talks about a folder / file / tab name, or
 *   (b) the folder label and the document entity share a distinctive token
 *       ("synergies"), which means it is the same tenant under a shorthand
 *       label rather than a genuine entity discrepancy.
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, folderName, onDrop }
 * @returns {Array<object>}  filtered findings
 */
export function dropFolderLabelNameMismatch(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag    = opts.tenantName ? `[name-label ${opts.tenantName}]` : `[name-label]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null
  const labelTokens = new Set([
    ...entityTokens(opts.folderName),
    ...entityTokens(opts.tenantName)
  ])

  const kept = []
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    if (!checkType.includes('NAME_MISMATCH') && !checkType.includes('TENANT_NAME')) {
      kept.push(f); continue
    }

    const txt = `${f.missingDocument || ''} ${f.comment || ''} ${f.issue || ''} ${f.description || ''}`.toLowerCase()

    // (a) The finding openly grounds itself in an administrative label.
    if (/\b(folder|file ?name|filename|tab name|folder label|folder name|directory)\b/.test(txt)) {
      console.log(`${tag} DROP — compares an administrative label to an entity name`)
      if (onDrop) try { onDrop(f, 'folder-or-filename vs entity name') } catch {}
      continue
    }

    // (b) The label and the document entity share a distinctive token.
    if (labelTokens.size > 0) {
      const shared = [...labelTokens].filter(t => txt.includes(t))
      if (shared.length > 0) {
        console.log(`${tag} DROP — label shares distinctive token(s) [${shared.join(', ')}] with the document entity`)
        if (onDrop) try { onDrop(f, `shared entity token: ${shared.join(', ')}`) } catch {}
        continue
      }
    }
  ...
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after folder-label name check`)
  }
  return kept
}

/**
 * Drops MISSING_DOCUMENT / REFERENCED_DOC findings for an instrument that the
 * referencing text itself says has been TERMINATED, EXPIRED or SUPERSEDED.
 *
 * Recitals routinely name a prior agreement for the express purpose of retiring
 * it, which reads to the model like a textbook CHECK 4 hit. Regression this
 * guards (blind loop 1, 2026-08-04, Bank of Houston): Rider No. 4 §E reads
 * "the term of that certain Temporary Lease Agreement dated May 9, 2018 ...
 * terminates pursuant to its terms", and the model flagged it as missing.
 *
 * Only fires when the finding's OWN text acknowledges the termination — the
 * model usually quotes the recital it relied on, so the evidence travels with
 * the finding.
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, onDrop }
 * @returns {Array<object>}  filtered findings
 */
export function dropTerminatedReferencedDocs(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag    = opts.tenantName ? `[terminated-ref ${opts.tenantName}]` : `[terminated-ref]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null
  ...
  const TERMINATION_PHRASES = [
    ...
    'was hereby terminated',
    'expired by its terms',
    'expires by its terms',
    'is superseded and replaced',
    'superseded and replaced by',
    'shall automatically terminate',
    'automatically terminates upon',
    'terminated as of the commencement'
  ]

  const kept = []
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    const isRefCheck = checkType.includes('MISSING_DOCUMENT') ||
                       checkType.includes('REFERENCED_DOC')
    if (!isRefCheck) { kept.push(f); continue }

    const txt = `${f.missingDocument || ''} ${f.comment || ''} ${f.issue || ''} ${f.description || ''}`.toLowerCase()
    const hit = TERMINATION_PHRASES.find(p => txt.includes(p))
    if (hit) {
      console.log(`${tag} DROP — referencing text says the document is retired ("${hit}") — ${(f.missingDocument || '').slice(0, 90)}`)
      if (onDrop) try { onDrop(f, `terminated/superseded: "${hit}"`) } catch {}
      continue
    }
    kept.push(f)
  }
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after terminated-reference check`)
  }
  return kept
}

// Form exhibits that are bound into a lease as blank templates. Their blankness
// is their normal state — they are filled in later, or never. The ONLY exhibit
// whose execution is ever checked is a Guaranty.
const FORM_EXHIBIT_NAMES = [
  'commencement letter',
...
// Language that marks a reference as CONTEMPLATED rather than EXISTING.
const CONTEMPLATED_PHRASES = [
  ...
  'was never completed',
  'template that has not been'
]

/**
 * L26 — PAST-TENSE TEST. Drops findings about documents the lease merely
 * CONTEMPLATES (blank form exhibits) rather than recites as already existing.
 *
 * A blank Commencement Letter / Estoppel / SNDA form bound into the lease is a
 * template, not a defect. Crucially this filter is checkType-AGNOSTIC across
 * the four framings the model rotates through, because relabeling the same
 * observation does not make it valid:
 *   MISSING_DOCUMENT  "the completed Commencement Letter is absent"
 *   EXECUTION         "both By: lines on Exhibit C are blank"
 *   MISSING_EXHIBIT   "the executed Exhibit C is missing"
 *   LEASE_CURRENCY    "without it the expiration date cannot be confirmed"
 *
 * Guaranty findings are NEVER dropped — a Guaranty is the one exhibit whose
 * execution the reviewer does want checked.
 *
 * Regression this guards: FP83-19 Financial Synergies, lawyer test 2026-08-03 —
 * "The only exhibit that should be executed are guaranties. Many exhibits are
 * just forms." Also FP83-14 Eagle LNG (body reference to a form exhibit that
 * the index never lists).
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, onDrop }
 * @returns {Array<object>}
 */
export function dropContemplatedFormDocs(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag    = opts.tenantName ? `[form-exhibit ${opts.tenantName}]` : `[form-exhibit]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null

  const kept = []
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    const inScope = checkType.includes('MISSING_DOCUMENT') ||
                    checkType.includes('MISSING_EXHIBIT')  ||
                    checkType.includes('EXECUTION')        ||
                    checkType.includes('LEASE_CURRENCY')   ||
                    checkType.includes('REFERENCED_DOC')
    if (!inScope) { kept.push(f); continue }

    const txt = `${f.missingDocument || ''} ${f.comment || ''} ${f.issue || ''} ${f.description || ''} ${f.evidence || ''} ${f.triggerQuote || ''}`.toLowerCase()

    // NEVER suppress a Guaranty — the one exhibit whose execution IS checked.
    if (txt.includes('guaranty') || txt.includes('guarantee') || txt.includes('guarantor')) {
      // ...unless it is explicitly the blank FORM of a guaranty rather than an
      // executed guaranty that should exist.
      if (!txt.includes('form of guaranty')) { kept.push(f); continue }
    }

    const nameHit = FORM_EXHIBIT_NAMES.find(n => txt.includes(n))
    if (!nameHit) { kept.push(f); continue }

    // The document name alone isn't enough — a fully executed Commencement
    // Date Agreement that a later amendment recites by date IS a real missing
    // document. Require corroboration that this is the blank-template case.
    const contemplatedHit = CONTEMPLATED_PHRASES.find(p => txt.includes(p))
    const looksBlank      = /\bblank\b/.test(txt) || /\bunexecuted form\b/.test(txt) ||
                            /\btemplate\b/.test(txt) || /\bnot been filled\b/.test(txt) ||
                            /\bnever\s+(?:been\s+)?(?:filled|completed|executed)\b/.test(txt)
    // A specific recited date is the strongest signal the document really exists.
    const hasRecitedDate  = /\bdated\s+(?:as of\s+)?(?:\w+\s+\d{1,2},?\s+\d{4}|\d{1,2}\/\d{1,2}\/\d{2,4})/.test(txt)

    if ((contemplatedHit || looksBlank) && !hasRecitedDate) {
      const why = contemplatedHit ? `contemplated-only ("${contemplatedHit}")` : 'blank form exhibit'
      console.log(`${tag} DROP — ${why}: "${nameHit}" — ${(f.missingDocument || '').slice(0, 90)}`)
      if (onDrop) try { onDrop(f, `L26 blank/contemplated form exhibit: "${nameHit}"`) } catch {}
      continue
    }
    kept.push(f)
  }
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after form-exhibit check`)
  }
  return kept
}

// Entity-level instruments. These govern the CORPORATE ENTITY, not the tenancy,
// and never belong in a landlord's lease file.
const CORPORATE_DOC_PATTERNS = [
  /\bbank merger agreement\b/,
  /\bmerger agreement\b/,
  /\bagreement (?:and )?plan of merger\b/,
  /\bplan of merger\b/,
  /\bcertificate of merger\b/,
  /\bplan of reorganization\b/,
  /\bstock purchase agreement\b/,
  /\basset purchase agreement\b/,
  /\bequity purchase agreement\b/,
  /\bmembership interest (?:purchase|transfer) agreement\b/,
  /\bpartnership interest (?:purchase|transfer) agreement\b/,
  /\bcertificate of conversion\b/,
  /\bloan agreement\b/,
  /\bcredit agreement\b/,
  /\bfinancing agreement\b/,
  /\bsecurity agreement\b/
]

/**
 * L27 — Corporate-level transaction documents are out of scope. Drops
 * MISSING_DOCUMENT / SPECIAL_AGREEMENT findings naming merger agreements,
 * purchase agreements, reorganization plans and the like.
 *
 * This holds even when the referencing document names the instrument AND gives
 * its effective date, and even when a lease clause requires the tenant to
 * deliver "all applicable documentation" of a transfer. A general delivery
 * covenant is not evidence that a corporate instrument sits in the lease file.
 *
 * What IS in scope is the lease-side instrument recording the change — the
 * Assignment and Assumption / Acknowledgement Agreement / Consent — which this
 * filter deliberately does not touch.
 *
 * Regression this guards: Bank of Houston, lawyer test 2026-08-03 — the model
 * flagged "Bank Merger Agreement effective December 1, 2025" as missing, on a
 * folder that already contained the executed Acknowledgement and Assumption
 * Agreement documenting the same merger. Related: FP83-13, where the reviewer
 * rejected the Acknowledgement itself as a Special Agreement with "This is not
 * considered a separate agreement."
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, onDrop }
 * @returns {Array<object>}
 */
export function dropCorporateTransactionDocs(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag    = opts.tenantName ? `[corp-doc ${opts.tenantName}]` : `[corp-doc]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null
  ...
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    const inScope = checkType.includes('MISSING_DOCUMENT') ||
                    checkType.includes('SPECIAL_AGREEMENT') ||
                    checkType.includes('REFERENCED_DOC')
    if (!inScope) { kept.push(f); continue }

    // Match on the document NAME field primarily — the comment may mention a
    // merger as background context for a legitimate finding.
    const name = `${f.missingDocument || ''} ${f.documentName || ''} ${f.issue || ''}`.toLowerCase()
    const hit  = CORPORATE_DOC_PATTERNS.find(p => p.test(name))
    if (hit) {
      console.log(`${tag} DROP — entity-level transaction document, not a lease document — ${(f.missingDocument || '').slice(0, 90)}`)
      if (onDrop) try { onDrop(f, `L27 corporate transaction document: ${hit.source}`) } catch {}
      continue
    }
    kept.push(f)
  }
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after corporate-document check`)
  }
  return kept
}

/**
 * Evidence that occupancy actually continues past the document chain's end
 * date. Any one of these makes a LEASE_CURRENCY finding legitimate even when
 * the chain end is in the future.
 */
const OCCUPANCY_BEYOND_MARKERS = [
  /\bholdover\b/, /\bhold over\b/, /\bholding over\b/,
  /\boption (?:to extend|to renew)[^.]{0,60}\bexercised\b/,
  /\brenewal[^.]{0,40}\bexercised\b/,
  /\bexercised[^.]{0,40}\b(?:option|renewal|extension)\b/,
  /\bstill in possession\b/, /\bremains in possession\b/,
  /\bcontinues to occupy\b/, /\bcurrently occupies\b/
]

/**
 * Clauses that mention an evidence source only to say it is ABSENT. These must
 * be stripped before testing OCCUPANCY_BEYOND_MARKERS, otherwise a finding that
 * explains "no rent-roll line was supplied, so I used the fallback" reads as
 * though a rent roll were affirmative evidence of occupancy beyond the chain
 * end. Observed verbatim in the blind loop-3 output that prompted L28.
 */
const NEGATED_EVIDENCE_CLAUSES = [
  /\b(?:no|without|absent|lacking)\b[^.;]{0,40}\brent[- ]roll\b/gi,
  /\brent[- ]roll\b[^.;]{0,50}\b(?:not|never|n't)\s+(?:been\s+)?(?:supplied|provided|given|available|furnished|attached|included)\b/gi,
  /\bno[- ]rent[- ]roll\b/gi,
  /\b(?:no|without|absent)\b[^.;]{0,30}\b(?:holdover|evidence of (?:continued )?occupancy)\b/gi,
  /\b(?:option|renewal)[^.;]{0,40}\b(?:not|never)\s+(?:been\s+)?exercised\b/gi
]

/**
 * Affirmative signals that the chain's end date is an INTENDED hard stop
 * rather than a documentation hole.
 */
const INTENDED_TERMINUS_MARKERS = [
  /\btermination (?:letter|agreement|notice)\b/,
  /\bterminates pursuant to its terms\b/,
  /\bexpressly excludes?\b[^.]{0,80}\b(?:option to extend|rider no\.? 1|right of first refusal|renewal)\b/,
  /\b(?:option to extend|renewal (?:option|right)s?|extension rights?)\b[^.]{0,60}\b(?:excluded|deleted|do not apply|shall not apply|no longer apply)\b/,
  /\bdeliberately negotiated\b/, /\bintended hard stop\b/
]

/**
 * L28 — LEASE_CURRENCY is a RECONCILIATION finding, not a forecast.
 *
 * The check exists to catch files whose documents fail to support occupancy
 * that is ALREADY established. It is not an early-warning alarm for leases
 * that happen to expire soon. If the chain's end date is still in the future
 * and nothing in the record shows occupancy continuing past it, then no
 * document is missing — the instrument that would extend the term may not
 * exist yet, and asking an abstractor to chase a document that has not been
 * negotiated is a false positive.
 *
 * A finding therefore survives only when there is an EVIDENCED SHORTFALL:
 *   (a) the chain end is in the PAST (documents already lag reality), or
 *   (b) the finding describes an INTERNAL date range / gap (L10, L24), or
 *   (c) a rent roll End later than the chain end is supplied, or
 *   (d) the finding text cites evidence of occupancy beyond the chain end
 *       (rent roll, holdover, an exercised option, continued possession).
 *
 * Every confirmed LEASE_CURRENCY true positive in the 2026-08-03 reviewer set
 * satisfies (a) or (b): Amberjack "Term from 5/1/24-5/31/26" (internal gap,
 * both dates past), LOGIX "beyond 4/30/22", Century Oaks "beyond 10/31/2021".
 * None rests on a future expiration date.
 *
 * Regression this guards: Mayer (Blue Tide). The folder's newest instrument is
 * a two-month direct lease running 9/1/2026-10/31/2026 that was signed four
 * months ago and whose term has not yet commenced, with Rider No. 1 (Option to
 * Extend) and Rider No. 2 (ROFR) expressly excluded. The file is fully
 * current, yet the chain end sat 89 days out — one day inside the playbook's
 * "within 90 days" fallback — so the rule fired on a coin flip. Two blind runs
 * over identical facts computed the identical 89 days and reached OPPOSITE
 * conclusions, each logging it as its closest call. This filter removes the
 * ambiguity rather than picking a side.
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, today, rentRollEnd, onDrop }
 * @returns {Array<object>}
 */
export function dropUnevidencedForwardCurrency(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag    = opts.tenantName ? `[fwd-currency ${opts.tenantName}]` : `[fwd-currency]`
  const onDrop = typeof opts.onDrop === 'function' ? opts.onDrop : null
  ...
  const rentRollEnd = parseDate(opts.rentRollEnd)

  // All dates mentioned anywhere in the finding, so we can tell a single
  // "beyond X" endpoint from a "from A to B" range.
  const DATE_RE = /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+(?:19|20|21)\d{2}\b|\b\d{1,2}\/\d{1,2}\/(?:19|20|21)\d{2}\b|\b\d{1,2}\/\d{1,2}\/\d{2}\b/gi

  const kept = []
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    if (!(checkType.includes('LEASE_CURRENCY') || checkType === 'CURRENCY')) { kept.push(f); continue }

    const name = `${f.missingDocument || ''} ${f.documentName || ''} ${f.issue || ''}`
    const full = `${name} ${f.comment || ''}`
    // Strip "no rent roll was supplied"-style clauses so that a statement about
    // the ABSENCE of evidence is not mistaken for the evidence itself.
    let lower = full.toLowerCase()
    for (const p of NEGATED_EVIDENCE_CLAUSES) lower = lower.replace(p, ' ')

    // (b) An internal gap is phrased as a RANGE ("from A to B"). Those are
    // always retrospective coverage holes — never touch them.
    const nameDates = (name.match(DATE_RE) || [])
    const isRange = /\bfrom\b[^.]{0,40}\bto\b/i.test(name) || /\bbetween\b[^.]{0,40}\band\b/i.test(name) || nameDates.length > 1
    if (isRange) { kept.push(f); continue }

    // The chain-end date the finding is built on.
    const endDate = parseDate(nameDates[0]) ||
                    parseDate(((full.match(DATE_RE) || [])[0]) || null)

    // (a) Chain end already past → genuine documentation lag. Keep.
    // If we cannot parse a date at all, keep — never drop on ignorance.
    if (!endDate || endDate.getTime() <= today.getTime()) { kept.push(f); continue }

    // (c) A rent roll that runs past the chain end is direct proof of shortfall.
    if (rentRollEnd && rentRollEnd.getTime() > endDate.getTime()) { kept.push(f); continue }

    // (d) The finding itself cites evidence of occupancy beyond the chain end.
    if (OCCUPANCY_BEYOND_MARKERS.some(p => p.test(lower))) { kept.push(f); continue }

    // (d-bis) A rent roll named in the prose only counts as evidence when the
    // text also carries a date LATER than the chain end — i.e. it actually
    // asserts a shortfall. Merely naming the rent roll proves nothing: the
    // loop-3 output that prompted this rule said "no rent-roll line was
    // supplied ... tested against today's date rather than against a rent-roll
    // End column", which a bare keyword match reads as evidence of the very
    // thing it denies. Requiring a later date is negation-proof, where
    // enumerating negation phrasings is not.
    if (/\brent[- ]roll\b/.test(lower)) {
      const laterDate = (full.match(DATE_RE) || [])
        .map(parseDate)
        .some(d => d && d.getTime() > endDate.getTime())
      if (laterDate) { kept.push(f); continue }
    }

    const days = Math.round((endDate.getTime() - today.getTime()) / 86400000)
    const intended = INTENDED_TERMINUS_MARKERS.some(p => p.test(lower))
    console.log(`${tag} DROP — chain end ${endDate.toISOString().slice(0, 10)} is ${days} days in the FUTURE with no evidence of occupancy beyond it${intended ? ' (and the record shows the end date is intended)' : ''} — ${name.trim().slice(0, 90)}`)
    if (onDrop) try { onDrop(f, `L28 forward-looking currency, no evidenced shortfall (${days}d out)`) } catch {}
  }
  if (kept.length !== findings.length) {
    console.log(`${tag} kept ${kept.length} / ${findings.length} findings after forward-currency check`)
  }
  return kept
}

/**
 * Drops CURRENCY findings whose "expiring within 90 days" claim doesn't match
 * the math. Many findings say "NOTE: Expiring within 90 days" but the
 * referenced expiration date is more than 90 days from today. This filter
 * catches that arithmetic error and drops the finding.
 *
 * @param {Array<object>} findings
 * @param {object} [opts]  { tenantName, today }  today defaults to now
 * @returns {Array<object>}  filtered findings
 */
export function dropBadDateMathFindings(findings, opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  const tag = opts.tenantName ? `[date-math ${opts.tenantName}]` : `[date-math]`
  const today = opts.today instanceof Date ? opts.today : new Date()
  const ninetyDaysMs = 90 * 24 * 60 * 60 * 1000
  const kept = []
  for (const f of findings) {
    const checkType = (f.checkType || '').toUpperCase()
    const text = ((f.missingDocument || '') + ' ' + (f.comment || '')).toUpperCase()
    const claimsWithin90 = /EXPIRING WITHIN 90 DAYS|EXPIRES WITHIN 90 DAYS|LESS THAN 90 DAYS/.test(text)
    if (!claimsWithin90 || !(checkType.includes('CURRENCY') || checkType.includes('LEASE_CURRENCY'))) {
      kept.push(f)
      continue
    }
    // Find a year in 20XX or 21XX in the finding text. Take the latest year mentioned.
    const yearMatch = (f.missingDocument || '').match(/20\d{2}|21\d{2}/g) || []
    const fromComment = (f.comment || '').match(/20\d{2}|21\d{2}/g) || []
    const allYears = [...yearMatch, ...fromComment].map(y => parseInt(y, 10)).filter(y => y >= 2020 && y <= 2100)
    if (allYears.length === 0) {
      // Cannot validate — keep finding (don't drop unless we have evidence the math is wrong)
      kept.push(f)
      continue
    }
    // Use the year that appears nearest to "expiration", "extending Term beyond", or "expires"
    // Simpler heuristic: try to extract a full date pattern; fall back to year-only.
    const dateMatch = (f.missingDocument + ' ' + (f.comment || '')).match(
      /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+20\d{2}\b|\b\d{1,2}\/\d{1,2}\/20\d{2}\b/
    )
    ...
    if (!expirationDate) {
      // Fall back to year-end of the latest year mentioned (worst case for keeping the finding)
      const maxYear = Math.max(...allYears)
      expirationDate = new Date(maxYear, 11, 31)
    }
    const gapMs = expirationDate.getTime() - today.getTime()
    if (gapMs > ninetyDaysMs) {
      const gapDays = Math.round(gapMs / (24 * 60 * 60 * 1000))
      const preview = (f.missingDocument || '').slice(0, 100)
      console.log(`${tag} DROP — claimed "within 90 days" but expiration ${expirationDate.toISOString().slice(0, 10)} is ${gapDays} days from today — ${preview}`)
      if (typeof opts.onDrop === 'function') try { opts.onDrop(f, `gap_${gapDays}d_>${90}d`) } catch {}
      continue
    }
    kept.push(f)
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// FOLDER MANIFEST CHECK — DROP FINDINGS WHEN THE REFERENCED DOC IS ACTUALLY
// IN THE FOLDER (filename match)
// ═══════════════════════════════════════════════════════════════════════════
// The Todd filter (TP3) has this check. In TP2, when the model says "Lease
// dated February 1, 2025 is missing" but the folder contains a file named
// "Lease Agreement - Amazon - 4th Floor.pdf", that's a false positive — the
// file IS the referenced lease. This filter does a fuzzy match against the
// folder's filenames before letting the finding through.
//
// Match strategy: extract key tokens from finding.missingDocument (dates,
// document type keywords like "Lease", "Amendment", "Guaranty", "Notice",
// "Letter", etc.) and require an overlap of (date OR strong type keyword)
// in at least one filename.

const STRONG_TYPE_KEYWORDS = [
  'LEASE', 'AMENDMENT', 'AMEND', 'GUARANTY', 'GUARANTEE',
  'NOTICE', 'LETTER', 'AGREEMENT', 'ASSIGNMENT', 'EXTENSION',
  'COMMENCEMENT', 'CONSENT', 'WAIVER', 'RIDER', 'ESTOPPEL',
  'SUPPLEMENT', 'ABATEMENT', 'CONFIRMATION',
]
...
function extractDateTokens(s) {
  ...
  while ((m = monthRe.exec(t)) !== null) {
    tokens.push({ month: m[1].slice(0, 3).toLowerCase(), day: m[2], year: m[3] })
  }
  // Numeric m/d/yyyy or m-d-yyyy
  const numRe = /\b(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})\b/g
  while ((m = numRe.exec(t)) !== null) {
    tokens.push({ monthNum: m[1], day: m[2], year: m[3] })
  }
  // Just a 4-digit year (last resort)
  const yearRe = /\b(20\d{2}|21\d{2})\b/g
  while ((m = yearRe.exec(t)) !== null) {
    tokens.push({ year: m[1] })
  }
  return tokens
}

function dateInFilename(filename, dateTokens) {
  if (!dateTokens.length) return false
  const fn = String(filename || '').toLowerCase()
  for (const tok of dateTokens) {
    if (tok.month && tok.year && fn.includes(tok.month) && fn.includes(tok.year)) return true
    if (tok.monthNum && tok.day && tok.year && fn.includes(`${tok.monthNum.padStart(2, '0')}.${tok.day.padStart(2, '0')}.${tok.year}`)) return true
    if (tok.monthNum && tok.day && tok.year && fn.includes(`${tok.monthNum}-${tok.day}-${tok.year}`)) return true
    if (tok.monthNum && tok.day && tok.year && fn.includes(`${tok.monthNum}/${tok.day}/${tok.year}`)) return true
    if (tok.monthNum && tok.day && tok.year && fn.includes(`${tok.year}-${tok.monthNum.padStart(2, '0')}-${tok.day.padStart(2, '0')}`)) return true
  }
  return false
}

function typeKeywordMatch(filename, missingDocText) {
  const fn = String(filename || '').toUpperCase()
  const mdt = String(missingDocText || '').toUpperCase()
  for (const kw of STRONG_TYPE_KEYWORDS) {
    if (mdt.includes(kw) && fn.includes(kw)) return kw
  }
  return null
}

/**
 * Drops MISSING_DOCUMENT / MISSING_EXHIBIT / REFERENCED_DOC / AMENDMENT_GAP
 * findings whose referenced document plausibly matches a filename in the
 * folder's manifest. This catches the Amazon-style "Lease dated Feb 1, 2025
 * is missing" FP where the file is actually present as
 * "Lease Agreement - Amazon - 4th Floor.pdf".
 *
 * @param {Array<object>} findings
 * @param {Array<{ originalName?: string, name?: string }>} files
 * @param {object} [opts]  { tenantName }
 * @returns {Array<object>}  filtered findings
 */
export function dropFindingsInFolderManifest(findings, files = [], opts = {}) {
  if (!Array.isArray(findings) || findings.length === 0) return findings
  if (!Array.isArray(files) || files.length === 0) return findings
  const tag = opts.tenantName ? `[manifest ${opts.tenantName}]` : `[manifest]`
  ...
  for (const f of findings) {
    ...
    const isRefCheck = checkType.includes('MISSING_DOCUMENT') ||
                      checkType.includes('MISSING_EXHIBIT') ||
                      checkType.includes('REFERENCED_DOC') ||
                      checkType.includes('AMENDMENT_GAP')
    if (!isRefCheck) {
      kept.push(f)
      continue
    }
    const md = f.missingDocument || ''
    if (!md.trim()) {
      kept.push(f)
      continue
    }
    const dateTokens = extractDateTokens(md)
    let matchedFile = null
    let matchReason = null
    for (const fn of fileNames) {
      // Strongest signal: date in filename matches date in finding
      if (dateTokens.length && dateInFilename(fn, dateTokens)) {
        const kw = typeKeywordMatch(fn, md)
        if (kw) {
          matchedFile = fn
          matchReason = `date+keyword(${kw})`
          break
        }
      }
    }
    // Second pass: if no date match, try strong keyword match alone for very specific titles
    // (e.g. "Notice of Commencement Date" -> filename containing "Commencement Date Notice")
    if (!matchedFile) {
      const mdUpper = md.toUpperCase()
      for (const fn of fileNames) {
        const fnUpper = fn.toUpperCase()
        // Look for 2+ strong keywords from the missingDocument that appear in the filename
        const matchingKws = STRONG_TYPE_KEYWORDS.filter(kw => mdUpper.includes(kw) && fnUpper.includes(kw))
        if (matchingKws.length >= 2) {
          matchedFile = fn
          matchReason = `multi-keyword(${matchingKws.join(',')})`
          break
        }
      }
    }

    // Third pass: PRIMARY-DOCUMENT MATCH
    // When the finding is about a "primary" document — the original Lease,
    // a Sublease, an Agreement of Lease, an A&R Lease — and the folder
    // contains a file whose name matches that primary type (e.g. starts
    // with "Lease" or contains "Lease Agreement"), it's a match. This
    // catches the Amazon-style "Lease dated Feb 1 2025 is missing" FP
    // where the actual file is named "Lease Agreement - Amazon - 4th Floor".
    if (!matchedFile) {
      const mdUpper = md.toUpperCase().trim()
      // Detect primary-document phrasing in the finding text
      const primaryLeaseRe = /^(THE\s+|THAT\s+CERTAIN\s+)?(ORIGINAL\s+|MAIN\s+|UNDERLYING\s+|AMENDED\s+AND\s+RESTATED\s+|A&R\s+|A\s*&\s*R\s+)?(LEASE|AGREEMENT\s+OF\s+LEASE|LEASE\s+AGREEMENT|SUBLEASE)\b/
      const isPrimaryLeaseFinding = primaryLeaseRe.test(mdUpper)
      if (isPrimaryLeaseFinding) {
        for (const fn of fileNames) {
          const fnUpper = fn.toUpperCase()
          // Has LEASE in filename, but is NOT an Amendment-only file
          const hasLease = /\bLEASE\b/.test(fnUpper)
          const isAmendmentFile = /\bAMENDMENT\b|\bRIDER\b|\bSUPPLEMENT\b|\bNOTICE\b|\bCONSENT\b|\bWAIVER\b|\bGUARANTY\b/.test(fnUpper)
          if (hasLease && !isAmendmentFile) {
            matchedFile = fn
            matchReason = 'primary-lease-match'
            break
          }
        }
      }
      // Same for primary Guaranty
      if (!matchedFile && /^(THE\s+|EXECUTED\s+|EXISTING\s+)?(PARENT\s+|PERSONAL\s+|CORPORATE\s+|GOOD\s+GUY\s+)?(GUARANT[YE]E?)\b/.test(mdUpper)) {
        for (const fn of fileNames) {
          const fnUpper = fn.toUpperCase()
          if (/\bGUARANT[YE]E?\b/.test(fnUpper)) {
            matchedFile = fn
            matchReason = 'primary-guaranty-match'
            break
          }
        }
      }
    ...
    if (matchedFile) {
      const preview = md.slice(0, 100)
      console.log(`${tag} DROP — "${preview}" matches file "${matchedFile}" via ${matchReason}`)
      if (typeof opts.onDrop === 'function') try { opts.onDrop(f, `${matchReason}:${matchedFile}`) } catch {}
      continue
    }
    kept.push(f)
  }
...
function buildContent(tenant, pdfDocs, textDocs, pageGapFindings = [], batchInfo = null) {
  ...
  const allFiles = [
    ...allPdfNames.map(name => {
      ...
      const isChunk     = splitChunkNames.has(name)
      return `  - ${name} — PDF${inThisBatch ? ' (attached in this batch)' : ' (in separate batch — exists in folder)'}${isChunk ? ' ⚠️ PAGE-RANGE SLICE, NOT A SEPARATE DOCUMENT' : ''}`
    }),
    ...textDocs.map(d =>
  ...
  // ── Split-document reconciliation map ──────────────────────
  // PDFs over PDF_MAX_BYTES are cut into page-range chunks by the analyzer and
  // arrive here as separately-named "documents" (Lease_p1-13.pdf, _p14-26, ...).
  // Each chunk carries splitFrom = the original filename.
  const splitGroups = new Map()
  for (const d of pdfDocs) {
    if (!d.splitFrom) continue
    if (!splitGroups.has(d.splitFrom)) splitGroups.set(d.splitFrom, [])
    splitGroups.get(d.splitFrom).push(d)
  }
  const splitChunkNames = new Set([...splitGroups.values()].flat().map(d => d.filename))
...
The PDF documents are attached below. Read every single page of each PDF visually.`
  ...
  // ── Block 1a: Split-document notice (oversized PDF cut into page ranges) ──
  // Without this, the model treats each chunk as a standalone instrument: it
  // looks for an exhibit index inside "_p27-39", doesn't find one, and invents
  // the exhibits it EXPECTS a lease to have — then reports them missing.
  // Regression this guards: lawyer test 2026-08-03, Avison Young. One 48MB
  // lease was sliced into 4 chunks; the model hallucinated an "Exhibit A/B/C"
  // index (the real index is A-1, A-2, B, C, D, E, E-1 + Riders 1-3, and it
  // sits on PDF page 2 of the FIRST chunk), then filed 3 MISSING_EXHIBIT
  // findings plus a GUARANTY finding on a lease whose face reads
  // "Guarantor(s): None". 4 of the session's 19 false positives, one cause.
  if (splitGroups.size > 0) {
    const groupLines = [...splitGroups.entries()].map(([orig, chunks]) => {
      const ordered = [...chunks].sort((a, b) => {
        const na = parseInt((a.filename.match(/_p(\d+)-/) || [])[1] || '0', 10)
        const nb = parseInt((b.filename.match(/_p(\d+)-/) || [])[1] || '0', 10)
        return na - nb
      })
      const totalPages = ordered.reduce((s, c) => s + (c.pageCount || 0), 0)
      return `ORIGINAL DOCUMENT: "${orig}" — ONE document, ${totalPages} pages total, delivered to you as ${ordered.length} slices:\n` +
        ordered.map(c => `    • ${c.filename}  (pages ${(c.filename.match(/_p([\d-]+)\.pdf$/) || [])[1] || '?'} of "${orig}")`).join('\n')
    }).join('\n\n')

    content.push({
      type: 'text',
      text: `${'═'.repeat(70)}
⚠️  SPLIT DOCUMENT NOTICE — READ BEFORE ANALYZING
${'═'.repeat(70)}
One or more PDFs in this folder exceeded the per-file size limit and were cut
into PAGE-RANGE SLICES by the file loader. The slices below are NOT separate
legal instruments. They are consecutive page ranges of a SINGLE document that
happens to be too large to transmit whole.

${groupLines}

HOW TO TREAT THESE SLICES:

1. RECONSTRUCT FIRST. Mentally concatenate the slices back into one document in
   page order before you evaluate anything. Every check runs against the
   REASSEMBLED document, never against an individual slice.

2. THE EXHIBIT INDEX LIVES IN EXACTLY ONE SLICE — USUALLY THE FIRST.
   An exhibit index / table of contents appears once, near the front. If the
   slice you are reading has no index, that means the index is in ANOTHER
   SLICE. It does NOT mean the document lacks an index.
   ⛔ NEVER infer, assume, reconstruct, or guess the contents of an exhibit
      index you have not physically read. If you cannot see the index, say so
      and emit NO exhibit finding.
   ⛔ NEVER assume a lease "should" have Exhibit A, B, C by convention. Read the
      actual index. Real indexes routinely use A-1, A-2, E-1, Rider No. 1, etc.

3. THESE CHECKS ARE FORBIDDEN ON A SINGLE SLICE — they are only valid on the
   fully reassembled document:
     • MISSING_EXHIBIT      (index and exhibits are usually in different slices)
     • MISSING_PAGES        (every slice boundary looks like a page jump)
     • AMENDMENT_GAP        (sequence spans the whole document)
     • GUARANTY             (the guaranty and the "Guarantor(s):" line are often
                             in different slices — read the lease's own
                             Guarantor field before concluding anything)
     • EXECUTION            (signature pages sit in whichever slice holds them —
                             a slice ending mid-document is not an unsigned one)

4. A SLICE BOUNDARY IS NOT A DEFECT. Printed page numbers restarting, an
   exhibit beginning mid-slice, or a document "ending" without signatures at a
   slice edge are all artifacts of the split. Never report them.

5. THE DOCUMENT'S TRUE DATE, PARTIES, AND TERM come from the reassembled
   document's own text — typically the first slice. Do not derive a document
   date from its FILENAME; filenames often encode a commencement date or a
   scan date, not the instrument date.
${'═'.repeat(70)}`
    })
  }
...
⚠️  CRITICAL FOR THE LEASE CURRENCY CHAIN WALK: "chainWalk" is how the merge
reconstructs the term chain across all ${batchInfo.totalBatches} batches. A hole between the last
term-bearing document of one batch and the first of another is invisible to
every batch — including this one — and is found only by stitching these rows
together afterwards. Emit one chainWalk row with BOTH "coversFrom" and
"coversThrough" for every term-bearing document you can see (including ones you
know only from other-batch extracted text), even when your own walk shows no gap.
Omitting rows silently blinds that pass.
${'═'.repeat(70)}`
    })
  }
...
🚨 COUNTERPART SIGNATURE PAGES — MANDATORY SEARCH BEFORE FLAGGING 🚨
- Before flagging EXECUTION on the main body of a Lease, Amendment, or Guaranty, you MUST search the ±3 pages around the apparent signature page for a counterpart signature page bearing the OPPOSITE party's signature.
- Sophisticated commercial leases (NYC: Park Avenue Plaza, MIDWEST: GKI portfolios, BRIXMOR Northwood Plaza, etc.) commonly use back-to-back counterpart pages — one with Landlord signature only, one with Tenant signature only.
- Two complementary counterpart pages together = full execution. Only flag EXECUTION when, after searching both directions, NO counterpart signature exists anywhere in the document.

🚨 PRESUME ORIGINAL-LEASE EXECUTION FOR OPERATING TENANTS 🚨
- When the rent roll shows the tenant is currently on lease (Begin date matches lease commencement, tenant is paying rent and operating), the original main-lease Landlord/Tenant signature block is PRESUMED — do NOT flag a blank By: line on the original main lease signature page as an EXECUTION finding.
- The signature is assumed to exist via a separately-stored counterpart held in a different file.
- This rule applies ONLY to the original main-lease signature blocks on a lease whose Term has already commenced. Guaranty / Amendment / Letter Agreement / Rent Commencement Date Agreement / Substitute Guaranty / Release of Guaranty execution is ALWAYS checked.
...
CRITICAL OUTPUT RULE — EXPLICIT DATE MATH REQUIRED:
1. Identify TODAY: ${todayStr} (this is the reference date — do not use any other date)
2. Identify the controlling expiration date from the most recent operative document in the chain
3. Compute the gap in days: gap_days = (controlling_expiration_date − today)

4. WALK THE CHAIN FOR INTERNAL GAPS - A SEPARATE AND MANDATORY COMPUTATION.
   The arithmetic above asks only whether the chain reaches far enough FORWARD.
   It says nothing about holes BEHIND that endpoint. Order every term-bearing
   document by date and, for each consecutive pair, compute the days between the
   earlier document's term end and the later document's term start. Record every
   interval in "chainGaps", including intervals you expect to drop. A file can
   carry a perfectly acceptable controlling expiration date and still contain a
   multi-year uncovered period earlier in its chain; reasoning only about the
   endpoint is exactly how those go unreported. Compute the intervals BEFORE
   forming any view about whether they matter.

   RECORD THE WALK ITSELF IN "chainWalk" — ONE ROW PER TERM-BEARING DOCUMENT,
   WITH BOTH ENDS OF THE PERIOD IT COVERS. Every row needs "coversFrom" (the
   date that document's term or extended period BEGINS) as well as
   "coversThrough" (the date it runs to). Use YYYY-MM-DD. Write null for an end
   the file never establishes — do not guess one, and do not omit the row.
   Emit these rows even when your walk finds no gap at all: they are what lets
   the chain be re-checked against documents you were not shown. If this folder
   is being processed in multiple batches (you are told when it is), the walk you
   record here covers only the documents in front of you, and a post-merge pass
   stitches every batch's walk together to find holes that straddle the batch
   boundary — a hole neither batch can see on its own. An incomplete chainWalk
   silently blinds that pass, so record a row for every term-bearing document you
   can see, including ones you know only from other-batch extracted text.

   An uncovered interval longer than 6 months is its own LEASE CURRENCY finding,
   worded "Document extending Term from [X] to [Y minus 1 day]", and it is emitted
   IN ADDITION TO any missing-document findings naming instruments believed to
   belong in that hole - UNLESS the instruments you named would actually SPAN the
   interval. Numbered placeholders (such as "Amendment No. 1, if any" sitting
   between an original lease and an in-folder Amendment No. 3) DO span it by
   construction, so no companion finding is written there. An instrument dated
   BEFORE the interval opens, one whose term the file never establishes, or a
   collateral instrument such as a sublease, consent or guaranty does NOT span it,
   and the interval finding stands alongside the missing-document findings. If you
   are genuinely unsure whether what you named covers the period, EMIT the
   interval finding.

THIS IS A RECONCILIATION CHECK, NOT A FORECAST. Its purpose is to catch a file
whose documents fail to support occupancy that is ALREADY ESTABLISHED. It is
NOT an early-warning alarm for leases that happen to expire soon.

Then apply EXACTLY these rules:
- If gap_days ≤ 0 (the controlling expiration is in the PAST, or is today): flag
  "Document extending Term beyond [most recent expiration date]." The documents
  demonstrably lag reality. This case is self-proving — no other evidence needed.
- If gap_days > 0 (the controlling expiration is in the FUTURE): OUTPUT NOTHING,
  UNLESS you can point to affirmative evidence that occupancy continues PAST that
  date. Qualifying evidence, and nothing else:
    (a) a rent roll End column later than the controlling expiration date;
    (b) the tenant is in holdover;
    (c) an option to extend or renew was EXERCISED but no amendment documents it;
    (d) a document in the folder recites occupancy past that date (e.g. a sublease
        running longer than the prime term, or a recital that Tenant remains in
        possession).
- If in holdover with no formal extension: flag this.

⚠️ PROXIMITY IS NOT EVIDENCE. An expiration date that is merely APPROACHING is
not a finding, whether it is 890 days away or 89. Do NOT flag "expiring within
90 days" as a finding in itself — that trigger has been WITHDRAWN because it
produced opposite answers on identical facts depending on which side of an
arbitrary cutoff the arithmetic landed. The document that would extend the term
may simply not have been negotiated yet, and a lease approaching its end date
with nothing contradicting it is a CURRENT, COMPLETE file.

⚠️ SIGNALS THE END DATE IS AN INTENDED HARD STOP (confirm the silence): a
termination letter or notice closing the tenancy; a short-term or bridge lease
whose text EXPRESSLY EXCLUDES the extension machinery (Option to Extend, Right
of First Refusal, renewal riders); or a recital that such rights "shall not
apply." When you see these, the end date is a negotiated terminus, not a
documentation hole. THESE SIGNALS SPEAK ONLY TO THE FORWARD ENDPOINT. A
termination notice, a negotiated terminus, or an excluded renewal rider says
nothing whatever about an uncovered interval EARLIER in the chain, and must
never be used to suppress an internal-gap finding under step 4 above. A file can
end on a clean negotiated hard stop and still have a multi-year hole behind it.

⚠️ DATE-MATH SANITY CHECK BEFORE EMITTING ANY CURRENCY FINDING:
- Re-state today's date: ${todayStr}
- Re-state the expiration date you identified.
- Re-compute gap_days as integer days between them.
- If gap_days > 0, name which of (a)–(d) above you are relying on. If you cannot
  name one, DO NOT EMIT THE FINDING.

Only flag if something is WRONG. A lease with a clear expiration date still in
the future, and no evidence of occupancy beyond it, is NOT a finding.
...

{
  ...
  "chainWalk": [
    { "doc": "document name and date", "coversFrom": "YYYY-MM-DD or null", "coversThrough": "YYYY-MM-DD or null" }
  ],
  "chainGaps": [
    { "between": "earlier document term end (date)", "and": "later document term start (date)", "uncoveredDays": 0 }
  ],
  "findings": [
    {
...
async function callWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  ...
  // maxTokensOverride is set only by the truncation handler below, which
  // re-issues the call with a bigger output budget. Hard-capped so a runaway
  // doubling loop can't ask the API for an illegal ceiling.
  const max_tokens = options.maxTokensOverride
    ? Math.min(options.maxTokensOverride, cheap ? 8192 : 32768)
    : pickClaudeMaxTokens(cheap, 16384)
  try {
    ...
    // ── OUTPUT TRUNCATION (stop_reason === 'max_tokens') ─────────────────────
    // The model ran out of output budget mid-JSON. The text is valid up to the
    // cut point and then just stops — e.g. `"missingDocument": "Interest an`.
    // This is NOT a malformed-response problem, so the parse-failure retry
    // below re-issues an identical request that truncates at the same point.
    // Raise the ceiling and try once; if it truncates again, salvage the
    // findings that did come through complete rather than losing the tenant.
    // Regression this guards: Comcast, lawyer test 2026-08-03 — the reviewer
    // got a "manual review required" row and no analysis at all.
    if (response.stop_reason === 'max_tokens') {
      if (!options.maxTokensOverride && !cheap) {
        const raisedCeiling = max_tokens * 2
        console.warn(`[claude] ${tenant.tenantName}: output truncated at ${max_tokens} tokens — retrying at ${raisedCeiling}`)
        return callWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo,
          { ...options, maxTokensOverride: raisedCeiling })
      }
      const salvaged = _salvageTruncatedFindings(rawText)
      if (salvaged && salvaged.findings.length > 0) {
        console.warn(`[claude] ${tenant.tenantName}: still truncated at ${max_tokens} — salvaged ${salvaged.findings.length} complete finding(s)`)
        return salvaged
      }
    }
...
function parseResponse(text, tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  // Use the same tolerant parser as gym mode. The old bare JSON.parse here
  // failed on ```json fences and trailing prose that gym mode had already
  // learned to handle, sending recoverable responses down the retry path.
  const parsed = _tolerantJsonParse(text)
  if (parsed && parsed.findings && Array.isArray(parsed.findings)) {
    return parsed
  }

  // Before burning a retry, see whether this is a truncation we can salvage.
  // (callWithRetry catches stop_reason === 'max_tokens' directly, but a
  // truncated body can also arrive without that flag on some failover paths.)
  const salvaged = _salvageTruncatedFindings(text)
  if (salvaged && salvaged.findings.length > 0) {
    console.warn(`[claude] ${tenant.tenantName}: unparseable response looked truncated — salvaged ${salvaged.findings.length} complete finding(s)`)
    return salvaged
  }
  ...
  // Surface as a SYSTEM_ERROR row, NOT a HIGH LEGIBILITY finding. The reviewer
  // rejects legibility rows that are really infrastructure failures: "It's good
  // to note that it needs manual review, but it's more helpful for it to be
  // reviewed by the app." SYSTEM_ERROR is visually distinct and Todd's filter
  // can drop it, whereas a HIGH LEGIBILITY row reads as a document defect.
  return {
    ...
    findings: [{
      ...
      comment:         'The model returned a response that could not be parsed after multiple retries. Re-running the tenant usually fixes it.',
...
async function gymCallWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  const cheap = !!options.cheapMode
  const model = pickClaudeModel(cheap)
  // A prior truncated attempt can raise the ceiling for this retry (see the
  // stop_reason === 'max_tokens' handler below).
  const max_tokens = options.maxTokensOverride
    ? Math.min(options.maxTokensOverride, cheap ? 8192 : 32768)
    : pickClaudeMaxTokens(cheap, 16384)
  try {
    ...
    // ── OUTPUT TRUNCATION (stop_reason === 'max_tokens') ───────────────────
    // The model ran out of output budget mid-JSON. The text is valid up to the
    // cut point and then simply stops — e.g. `"missingDocument": "Interest an`.
    // This is NOT a malformed-response problem, so re-issuing the identical
    // request fails identically, which is why the old retry loop always ended
    // in a SYSTEM_ERROR row. Raise the ceiling and try once more; if it is
    // still truncated, salvage the findings that did come through complete.
    // Regression this guards: Comcast, lawyer test 2026-08-03 — SYSTEM_ERROR
    // on every attempt, and the reviewer saw an empty audit for the tenant.
    if (response.stop_reason === 'max_tokens') {
      const raisedCeiling = max_tokens * 2
      if (!options.maxTokensOverride && !cheap) {
        console.warn(`[claude/gym] ${tenant.tenantName}: output truncated at ${max_tokens} tokens — retrying at ${raisedCeiling}`)
        return gymCallWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo,
          { ...options, maxTokensOverride: raisedCeiling })
      }
      const salvaged = _salvageTruncatedFindings(rawText)
      if (salvaged && salvaged.findings.length > 0) {
        console.warn(`[claude/gym] ${tenant.tenantName}: still truncated at ${max_tokens} — salvaged ${salvaged.findings.length} complete finding(s)`)
        if (usage) {
          salvaged._tokenUsage = {
...
/**
 * Salvage the complete findings out of a response that was cut off mid-JSON
 * because the model hit its output-token ceiling.
 *
 * The text is perfectly valid right up to the cut point and then simply stops —
 * e.g. `..., "missingDocument": "Interest an`. JSON.parse rejects the whole
 * thing, so _tolerantJsonParse returns null and the caller emits a SYSTEM_ERROR
 * row. But the findings that were emitted BEFORE the cut are complete and
 * usable, and throwing them away is what left the reviewer with an empty audit.
 *
 * Strategy: locate the `"findings"` array, then walk it tracking brace depth
 * and string/escape state, keeping every object that closes cleanly. Header
 * fields (tenantNameInDocuments etc.) are scraped separately because they are
 * emitted before the findings array and therefore always survive truncation.
 *
 * Regression this guards: Comcast, lawyer test 2026-08-03.
 *
 * @param {string} text  Raw (truncated) model output
 * @returns {{tenantNameInDocuments, mostRecentDocumentDate, leaseExpirationDate, findings: Array, allClear: boolean}|null}
 */
function _salvageTruncatedFindings(text) {
  if (!text || typeof text !== 'string') return null
  ...
  // ── Locate the findings array ────────────────────────────────────────────
  const keyMatch = work.match(/"findings"\s*:\s*\[/)
  if (!keyMatch) return null
  const arrStart = keyMatch.index + keyMatch[0].length

  // ── Walk the array, collecting only objects that close cleanly ───────────
  const objects = []
  let depth = 0, objStart = -1, inString = false, escape = false
  for (let i = arrStart; i < work.length; i++) {
    const c = work[i]
    if (escape) { escape = false; continue }
    if (c === '\\') { escape = true; continue }
    if (c === '"') { inString = !inString; continue }
    ...
    if (c === '{') {
      ...
      if (depth === 0 && objStart >= 0) {
        objects.push(work.substring(objStart, i + 1))
        objStart = -1
      } else if (depth < 0) {
        break   // hit the array's closing ] region / outer object — stop
      }
    } else if (c === ']' && depth === 0) {
      break     // array closed normally — nothing left to salvage here
    }
  }
  ...
  for (const raw of objects) {
    ...
    try { obj = JSON.parse(raw) } catch {
      try { obj = JSON.parse(raw.replace(/,(\s*[}\]])/g, '$1')) } catch { obj = null }
    }
    // Only keep findings that carry enough substance to be actionable.
    if (obj && typeof obj === 'object' && (obj.checkType || obj.check_type)) findings.push(obj)
  }

  if (findings.length === 0) return null

  // ── Scrape the header fields (emitted before findings, so never truncated) ─
  const scrape = (key) => {
    const m = work.match(new RegExp(`"${key}"\\s*:\\s*(?:"((?:[^"\\\\]|\\\\.)*)"|null)`))
    if (!m) return null
  ...
  return {
    tenantNameInDocuments:  scrape('tenantNameInDocuments'),
    mostRecentDocumentDate: scrape('mostRecentDocumentDate'),
    leaseExpirationDate:    scrape('leaseExpirationDate'),
    findings,
    allClear: false,
    _truncationSalvage: true
  }
}
...
export async function synthesizeAcrossBatches(tenant, batchResults, allFileNames, options = {}, allPdfCandidates = []) {
  ...
  // Every batch's chainWalk stitched into one chain, with the uncovered
  // intervals already computed. A hole straddling a batch boundary was in no
  // single batch's request, so no batch could have reported it — this pass is
  // the first one that can see it, and it has the full text to rule on it.
  const chainCoverageBlock = formatChainCoverageForPrompt(
    batchResults.map((r, i) => ({ ...r, _batch: i + 1 }))
  )
...
C2) RULE ON EVERY UNCOVERED TERM INTERVAL. The merged term chain below was built
   from all ${batchResults.length} batches' chain walks and its uncovered intervals are already
   computed. An interval marked "SPANS A BATCH BOUNDARY" was in no single batch's
   request: neither batch could see both documents that bound it, so the silence
   of every batch on that interval is not evidence that it is covered.
   For each interval longer than ${MIN_REPORTABLE_GAP_DAYS} days, check the full document text below:
   • If some document in the folder actually covers the period, say nothing.
   • Otherwise emit a CURRENCY finding worded "Document extending Term from [X]
     to [Y minus 1 day]", where the period is the interval itself.
   Emit it IN ADDITION TO any missing-document finding naming an instrument
   believed to belong in that hole, UNLESS that instrument's own recited dates
   would actually SPAN the interval. An instrument dated before the interval
   opens, one whose term the file never establishes, and a collateral instrument
   (sublease, consent, guaranty, license) do NOT span it. If you are genuinely
   unsure whether what you named covers the period, EMIT the interval finding.
   A negotiated hard stop at the END of the chain says nothing about a hole
   behind it and never suppresses an interval finding.
...
══════════════════════════════════════════════
MERGED TERM CHAIN ACROSS ALL BATCHES (see C2):
══════════════════════════════════════════════
${chainCoverageBlock || '(no batch produced a chain walk — build the chain yourself from the full text below)'}
