 ...
 */
function nextClient() {
  if (_KEYS.length === 1) return new Anthropic({ apiKey: _KEYS[0] })
...
/**
 * messages.create — weighted schedule + 429 immediate failover.
 * On 429: fall back to key0 (highest capacity) unless key0 was the one that failed.
 */
async function createMessageWithFailover(params) {
  ...
  try {
    ...
    if (err?.status === 429 && _KEYS.length > 1) {
      const fallbackIdx = idx === 0 ? (1 % _KEYS.length) : 0
      console.warn(`[claude] 429 on key ${idx} → fallback to key ${fallbackIdx}`)
...
/**
 * beta.messages.create — weighted schedule + 429 immediate failover.
 * Concurrent tenant analyses spread across all keys per the tier-aware schedule.
 */
