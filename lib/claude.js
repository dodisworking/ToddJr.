...
/**
 * Make a single Anthropic messages.create call with automatic 429 key-rotation.
 * Falls back to the secondary key once before propagating the error.
 */
async function createMessageWithFailover(params) {
  try {
    ...
    if (err?.status === 429 && _KEYS.length > 1) {
      rotateKey()
      console.warn(`[claude] 429 on key ${_keyIdx - 1}, retrying with key ${_keyIdx % _KEYS.length}`)
      return await getClient().messages.create(params)
    }
    throw err
  }
