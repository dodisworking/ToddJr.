...
YOUR JOB:
Produce an updated complete rule set.
- For each MISSED finding: write a detection rule to catch that pattern next time.
- For each REJECTED (false positive) finding: write a narrow AVOID rule that prevents the same mistake — but make it conditional, not a blanket suppression.
- For each CONFIRMED finding: these were CORRECT — do NOT write any rule that would narrow, suppress, or reduce the likelihood of finding these. Treat them as a protected list.
...
- Never so broad that they suppress an entire check type or undo a confirmed correct finding
