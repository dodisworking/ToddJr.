...
- If gap_days > 90: lease is current. OUTPUT NOTHING. Silence = current. Do NOT write a note. Do NOT acknowledge currency. Do NOT flag "expiring within 90 days" unless the gap_days value is genuinely ≤ 90.
- If 0 < gap_days ≤ 90: flag "Document extending Term beyond [expiration date]. NOTE: Expiring within 90 days (gap = [gap_days] days from today ${todayStr})."
- If gap_days ≤ 0 (expired or expiring today): flag "Document extending Term beyond [most recent expiration date]."
...
- Only emit the finding if gap_days ≤ 90.
- Confidence-test: if the expiration date is in a year that is more than 1 year after today's year, gap_days CANNOT be ≤ 90. Do NOT emit the finding.

Only flag if something is WRONG. A current lease with a clear expiration date > 90 days away is NOT a finding.
