...
4. Write a trainingInstruction — a plain-English instruction a human can copy-paste directly to their AI assistant to add a generalizable training rule. It must:
   - Start with "Train the model:"
   - Describe the document evidence PATTERN (not this specific tenant's details)
   - Say what the model should do differently when it encounters that pattern
   - Be testable against any random tenant folder — NO tenant names, property names, or deal-specific details
   - Be 1-3 sentences maximum
   If the model got everything right (score 100), write: "No training needed — model performed correctly on this tenant."
...
Return ONLY valid JSON (no markdown, no prose):
{
  ...
  "trainingInstruction": "Train the model: [generalizable pattern-based rule, no tenant specifics, 1-3 sentences]",
  "score":               <integer 0-100>
}`
