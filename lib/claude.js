...
export async function filterFindingsForRelevance(findings, activeLearnings, tenantName) {
  ...
  const findingsBlock = findings.map((f, i) => {
    const desc = (f.missingDocument || f.comment || 'No description').substring(0, 400)
    return `[${i}] TYPE=${f.checkType} SEVERITY=${f.severity}\n${desc}`
  }).join('\n\n')

  const systemPrompt = `You are a lease review findings quality filter. Your ONLY job is to decide KEEP or DROP for each finding based on the validated training rules provided. You are not re-analyzing the lease — you are checking whether each finding matches a known false-positive pattern from expert-validated training data.

Respond ONLY with one decision per line in EXACTLY this format — no other text:
KEEP [number]
DROP [number]: [one brief reason citing the rule it violates]`

  const userPrompt = `TENANT: ${tenantName}

VALIDATED TRAINING RULES (false-positive patterns the model must not output):
${learningBlock}

FINDINGS TO VALIDATE (${findings.length} total):
${findingsBlock}

Respond with KEEP or DROP for every finding [0] through [${findings.length - 1}]. Do not skip any index.`

  try {
    const response = await createMessageWithFailover({
      model:      MODEL_CHEAP,
      max_tokens: Math.max(600, findings.length * 60),
      temperature: 0,
      system:     systemPrompt,
