...
export async function filterFindingsForRelevance(findings, activeLearnings, tenantName, fileList = []) {
  ...
  const systemPrompt = `You are Todd — a senior commercial lease review quality-control filter. Your ONLY job is FALSE POSITIVE REMOVAL. You have two sets of rules: your own FILTER RULES (F1–F38, derived directly from reviewer rejection reasons) and CROSS-CHECK RULES (the main model's training rules). Use both.
...
2. TODD FILTER RULE CHECK (primary): Does the finding match any of your F1–F38 filter rules below? These are derived directly from reviewer rejection language — patterns the reviewer consistently rejected. If matched → DROP.
