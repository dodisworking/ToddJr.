...
let _keyIdx = 0
function getClient() {
  return new Anthropic({ apiKey: _KEYS[_keyIdx % _KEYS.length] })
}
/** Rotate to next key (called on 429) */
function rotateKey() { _keyIdx++ }

const client = getClient()  // default client for streaming calls (uses primary key)

/**
 * messages.create with automatic 429 key-rotation (non-beta).
 */
async function createMessageWithFailover(params) {
  try {
    ...
    if (err?.status === 429 && _KEYS.length > 1) {
      rotateKey()
      console.warn(`[claude] 429 → rotating to key ${_keyIdx % _KEYS.length}`)
      return await getClient().messages.create(params)
    }
    throw err
  }
...
/**
 * beta.messages.create with automatic 429 key-rotation.
 * Used by all analysis calls (gym, standard, double-check).
 */
async function createBetaMessageWithFailover(params) {
  try {
    return await getClient().beta.messages.create(params)
  } catch (err) {
    if (err?.status === 429 && _KEYS.length > 1) {
      rotateKey()
      console.warn(`[claude] 429 → rotating to key ${_keyIdx % _KEYS.length}`)
...
async function callWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  ...
  try {
    ...
    // ── 429: rotate to backup key first, then retry immediately ─
    if (err.status === 429 && attempt < 3) {
      if (_KEYS.length > 1) {
        rotateKey()
        console.warn(`[claude] 429 for ${tenant.tenantName} → rotating to key ${_keyIdx % _KEYS.length}, retrying now`)
        return callWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt + 1, batchInfo, options)
      }
...
async function gymCallWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt, batchInfo = null, options = {}) {
  ...
  try {
    ...
    // 429: rotate to backup key and retry immediately
    if (err.status === 429 && attempt < 3) {
      if (_KEYS.length > 1) {
        rotateKey()
        console.warn(`[claude/gym] 429 for ${tenant.tenantName} → rotating to key ${_keyIdx % _KEYS.length}`)
        return gymCallWithRetry(tenant, pdfDocs, textDocs, pageGapFindings, attempt + 1, batchInfo, options)
      }
