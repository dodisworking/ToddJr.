...
For each finding, run these checks IN ORDER and DROP on the first match:

1. SELF-CONTRADICTION CHECK (highest priority): Does the finding's own text already dismiss or explain away the issue? Look for ANY of these phrases anywhere in the finding — including at the very END of a long comment:
   • "this is acceptable" / "this is acceptable shorthand"
   • "not a mismatch" / "not a deficiency" / "not a finding"
   • "executed version is operative"
   • "no missing document" / "fully executed"
   • "sufficient" / "no action needed" / "no concern"
   • "WITHDRAWING" / "WITHDRAWING THIS FINDING" / "I am withdrawing"
   • "WITHDRAWN" / "being WITHDRAWN" / "must be suppressed"
   • "NOT valid per Learning" / "per learning rule" / "per training rule"
   • "false positive" / "not actionable"
   If ANY of these phrases appear anywhere in the finding text (beginning, middle, or end) → DROP immediately. This is the single most wasteful false-positive pattern — the model writes a finding, then dismisses it in the same text, but still outputs it.

2. TRAINING RULE CHECK: Does the finding match any validated training rule below? Apply each rule as written — if the finding pattern matches → DROP. Use the TYPE field on each finding to focus first on rules with the matching check type, then scan GENERAL rules.

3. RELEVANCE CHECK: Even if no specific rule matches, ask: would a commercial lease reviewer actually need to act on this? Is it genuinely missing, genuinely unexecuted, genuinely a problem? If no action is needed → DROP.

4. REDUNDANCY CHECK (run AFTER evaluating all findings individually): Look across ALL findings together. If two or more findings describe the same underlying defect in the same document — even if worded differently or caught from different angles — keep only the single most complete and clearly worded one and DROP the rest. Common redundancy patterns to catch:
   • Same document flagged as unexecuted more than once (e.g., once from a blank signature block, once from a general execution summary)
   • Same missing document flagged as both MISSING_DOCUMENT and AMENDMENT_GAP
   • Same execution issue described twice because two pages of the same document both showed a blank signature block
   • Same document gap flagged once with evidence from body text and once from a recital reference
   When dropping a duplicate, always keep the finding with the clearest, most specific evidence and drop the vaguer or more general version.

KEEP only findings that describe a real, unresolved, non-duplicate issue requiring reviewer attention.
