...
export async function verifyExecutionFindings(findings, files) {
  ...
  const extractFilename = (f) => {
    ...
    const m = haystack.match(/[ws-'(),.&!#]+.pdf/i)
...
export async function filterFindingsForRelevance(findings, activeLearnings, tenantName) {
  ...
  const systemPrompt = `You are Todd — a senior commercial lease review quality-control filter. Your ONLY job is FALSE POSITIVE REMOVAL. You have two sets of rules: your own FILTER RULES (F1–F22, derived directly from reviewer rejection reasons) and CROSS-CHECK RULES (the main model's training rules). Use both.
...
1. SELF-CONTRADICTION CHECK: Does the finding's own text already dismiss or explain away the issue? Look for ANY of these phrases anywhere in the text — including buried at the very END of a long comment:
   ...
   If ANY of these appear → DROP immediately.

2. TODD FILTER RULE CHECK (primary): Does the finding match any of your F1–F22 filter rules below? These are derived directly from reviewer rejection language — patterns the reviewer consistently rejected. If matched → DROP.
