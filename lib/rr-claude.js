...
2. ARGUS ENTERPRISE RENT ROLL: "${argusName}"`
  ...
  // Build optional skip instructions from Secret Recipe
  let skipNote = ''
  ...
  if (enabledChecks && Array.isArray(enabledChecks) && enabledChecks.length < ALL_CHECKS.length) {
    const disabled = ALL_CHECKS.filter(c => !enabledChecks.includes(c))
    const SKIP_MAP = {
    ...
    if (skipLines.length) {
      skipNote = `\n\nSECRET RECIPE OVERRIDES — skip these checks:\n${skipLines.map(s => `- ${s}`).join('\n')}\n`
    }
  }
