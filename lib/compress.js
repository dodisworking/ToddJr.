...
export async function compressCandidates(candidates, targetDpi, onProgress = () => {}) {
  for (let i = 0; i < candidates.length; i++) {
    ...
    // Recover raw bytes from base64 or disk
    const buffer = c.base64
      ? Buffer.from(c.base64, 'base64')
      : (await import('fs')).default.readFileSync(c.diskPath)

    c.jpegPages      = await renderPdfToJpegs(buffer, targetDpi)
    c.compressedDpi  = targetDpi
    delete c.base64   // free memory — no longer needed
  }
}
