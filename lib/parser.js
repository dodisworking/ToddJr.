 ...
 *
 * @param {string|null} filePath  - Absolute path to the file, or null when inlineBuffer is provided
 * @param {string} originalName
 * @param {Buffer} [inlineBuffer] - If provided, use this instead of reading filePath
 * @returns {Promise<Array<{filename, gaps}>>}  gaps = [{afterLabel, beforeLabel, missing[]}]
 */
export async function detectPageNumberGaps(filePath, originalName, inlineBuffer) {
  if (!pdfParse) return []
  try {
    const buffer = inlineBuffer instanceof Buffer ? inlineBuffer : await readFile(filePath)
    const pageTexts = []
    ...
    // Pull the numbered pages, KEEPING their physical PDF page index.
    // The physical index is what lets us tell a real missing page apart from a
    // page whose printed number we simply failed to read.
    const numbered = []
    for (let pi = 0; pi < pageTexts.length; pi++) {
      const label = extractPageLabel(pageTexts[pi])
      if (label !== null) numbered.push({ label, physical: pi })
    }
    ...
    for (let i = 1; i < numbered.length; i++) {
      ...
      if (curr.label <= prev.label + 1) continue

      // ── PHYSICAL-PAGE RECONCILIATION ──────────────────────────────────────
      // A label jump from `prev.label` to `curr.label` claims that
      // (curr.label - prev.label - 1) pages are absent. But if that many
      // physical pages actually sit between the two labelled pages in the PDF,
      // nothing is missing at all — we just failed to OCR their page numbers.
      //
      // Regression this guards (lawyer test 2026-08-03, Avison Young):
      // labels read [... 8, 9, 10, <null>, 12, 13 ...]. PDF page 13 carries
      // printed page 11 but extractPageLabel() returned null for it, so the
      // detector reported "page 11 is missing" on a document where no page was
      // missing. Reviewer: "No pages are missing. Pgs 3-11 are present."
      const claimedMissing   = curr.label - prev.label - 1
      const physicalBetween  = curr.physical - prev.physical - 1
      if (physicalBetween >= claimedMissing) continue  // fully accounted for — not a gap

      // Only the shortfall is genuinely unaccounted for.
      const missing = []
      for (let m = prev.label + 1; m < curr.label; m++) missing.push(m)
      gaps.push({
        afterLabel: prev.label,
        beforeLabel: curr.label,
        missing,
        unaccountedPages: claimedMissing - physicalBetween,
        physicalPagesBetween: physicalBetween
      })
    }
 ...
 * @param {string|null} filePath   - Absolute path to the file on disk, or null when inlineBuffer is provided
 ...
 */
export async function extractText(filePath, originalName, inlineBuffer) {
  const ext = path.extname(originalName).toLowerCase()

  try {
    if (ext === '.pdf') {
      return await extractPdf(filePath, originalName, inlineBuffer)
    }

    if (ext === '.docx') {
      return await extractDocx(filePath, originalName, inlineBuffer)
    }

    if (['.txt', '.rtf', '.text'].includes(ext)) {
      const text = inlineBuffer
        ? inlineBuffer.toString('utf-8')
...
async function extractPdf(filePath, originalName, inlineBuffer) {
  if (!pdfParse) {
...
async function extractDocx(filePath, originalName, inlineBuffer) {
  const opts = (inlineBuffer instanceof Buffer) ? { buffer: inlineBuffer } : { path: filePath }
